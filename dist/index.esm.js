...
import wt, { useState as we, useRef as de, useEffect as ke, useMemo as ze, useCallback as ae, createContext as ri, useContext as ii, Component as ai, memo as Xa, forwardRef as Mn, useImperativeHandle as Ja } from "react";
...
const be = {
  ...
  COMPLETED: "completed",
  ERROR: "error"
}, Pr = (t) => t === be.SUBMITTED || t === be.STREAMING, uf = (t) => t === be.IDLE, hf = (t) => t === be.ERROR, df = (t) => t === Ke.PROCESSING, pf = (t) => t === Ke.COMPLETED, ff = (t) => t === Ke.ERROR;
var wo = /* @__PURE__ */ ((t) => (t.BRAND = "BRAND", t.ACCOUNT = "ACCOUNT", t.USER = "USER", t))(wo || {}), He = /* @__PURE__ */ ((t) => (t.DISCONNECTED = "disconnected", t.CONNECTING = "connecting", t.CONNECTED = "connected", t.RECONNECTING = "reconnecting", t))(He || {});
const So = {
...
class an {
  ...
  static connectionEstablished() {
    return this.createConnectionEvent(ft.CONNECTION_ESTABLISHED);
  }
  static connectionLost(e) {
    return this.createConnectionEvent(ft.CONNECTION_LOST, { reason: e });
  }
  static connectionRestored() {
    return this.createConnectionEvent(ft.CONNECTION_RESTORED);
  }
  static reconnecting(e, n) {
    return this.createConnectionEvent(ft.RECONNECTING, { attempt: e, maxAttempts: n });
  }
  static chatCompleted(e) {
    return this.createChatEvent(ft.CHAT_COMPLETED, { conversationId: e });
  }
  static chatError(e, n) {
    return this.createChatEvent(ft.CHAT_ERROR, { error: e, errorCode: n });
  }
}
class Ft {
  /**
  ...
  static createUpdateContextHelpersMessage(e) {
    return {
      type: xt.UPDATE_CONTEXT_HELPERS,
      contextHelpers: e
    };
  }
  /**
...
class xo {
  ...
  attemptReconnect() {
    ...
    if (console.log("[WebSocketManager] attemptReconnect called", {
      ...
      console.log("[WebSocketManager] Max reconnection attempts reached"), (s = this.onSystemEvent) == null || s.call(
        this,
        an.connectionLost("Max reconnection attempts reached")
    ...
    console.log("[WebSocketManager] Firing RECONNECTING event", {
      attempt: e,
      maxAttempts: n
    }), (o = this.onSystemEvent) == null || o.call(this, an.reconnecting(e, n));
  ...
  handleReconnectionOpened() {
    var e, n;
    this.updateConnectionState(!0, !1), this.justRefreshedTicket = !1, this.startHeartbeat(), (e = this.onSystemEvent) == null || e.call(this, an.connectionRestored()), (n = this.onOpen) == null || n.call(this);
  }
  ...
  sendHeartbeat() {
    const e = Ft.serializeHeartbeatPing();
    this.send(e);
  }
  stopHeartbeat() {
...
const ie = {
  ...
  isThinkingMessage: (t) => t.startsWith(ie.THINKING_PREFIX) || t.startsWith(ie.REASONING_PREFIX) || t.startsWith(ie.THOUGHT_PREFIX),
  isCompletedMessage: (t) => t.includes(ie.COMPLETED_MARKER),
  isErrorMessage: (t) => t.includes(ie.ERROR_MARKER),
  isHandlingMessage: (t) => t.includes(ie.HANDLING_MARKER),
  extractDuration: (t, e) => {
    const n = t.match(ie.PATTERNS.DURATION);
    if (!n) return;
    ...
    const i = r === 1 ? ie.UI_TEXT.DURATION_SECOND : ie.UI_TEXT.DURATION_SECONDS;
    return ` ${ie.UI_TEXT.DURATION_FOR} ${r} ${i}`;
  },
  cleanReasoningContent: (t) => {
    let e = t.replace(new RegExp(`^${ie.THINKING_PREFIX}\\s*`), "").replace(new RegExp(`^${ie.REASONING_PREFIX}\\s*`), "").replace(new RegExp(`^${ie.THOUGHT_PREFIX}\\s*`), "");
    return e = e.replace(/\s*for [\d.]+\s+\w+$/, ""), e = e.replace(ie.PATTERNS.THOUGHT_CONTENT, ""), e.trim();
  },
  getMessageType: (t, e) => e === !1 ? je.isErrorMessage(t) ? ie.MESSAGE_TYPES.ERROR : (je.isThinkingMessage(t) && ie.PATTERNS.DURATION.test(t) || je.isThinkingMessage(t), ie.MESSAGE_TYPES.THOUGHT) : je.isCompletedMessage(t) ? ie.MESSAGE_TYPES.COMPLETED : je.isErrorMessage(t) ? ie.MESSAGE_TYPES.ERROR : (je.isHandlingMessage(t) || je.isThinkingMessage(t) && !t.includes(ie.UI_TEXT.AI_IS_THINKING), ie.MESSAGE_TYPES.THINKING)
};
class To extends Qa {
  constructor(n) {
  ...
  handleReasoningEnd(n) {
    ...
    const o = i || ie.UI_TEXT.THOUGHT, c = `${ie.THOUGHT_PREFIX} ${o}${s}`;
    this.triggerReasoningUpdate(
      !1,
      c,
      r,
...
class Eo extends Qa {
  ...
  async handleToolCallRequest(n) {
    ...
    if (!this.processedToolCalls.has(r)) {
      this.processedToolCalls.add(r), (s = this.getHandler("onReasoningUpdate")) == null || s(!0, `${ie.HANDLING_MARKER} ${i}`, n);
      try {
        const u = await this.executeToolFunction(i, a);
        this.sendToolResponse(r, u), (o = this.getHandler("onReasoningUpdate")) == null || o(!1, `${ie.COMPLETED_MARKER} ${i}`, n);
      } catch (u) {
        this.sendToolError(r, u), (c = this.getHandler("onReasoningUpdate")) == null || c(!1, `${ie.ERROR_MARKER} Error: ${i} - ${u}`, n);
      }
    }
  }
  ...
  sendToolResponse(n, r) {
    if (!this.sendMessage)
      return;
    const i = Ft.serializeToolCallSuccess(n, r);
    this.sendMessage(i);
  }
  sendToolError(n, r) {
    if (!this.sendMessage)
      return;
    const i = r instanceof Error ? r.message : "Unknown error", a = Ft.serializeToolCallError(n, i);
    this.sendMessage(a);
  }
  handleServerToolCall(n) {
    ...
    if (r && ((i = n.toolName) != null && i.startsWith("lat_")) && n.toolCallId) {
      ...
      r(!0, `${ie.HANDLING_MARKER} ${n.toolName}`, a);
    }
  }
  handleServerToolResult(n) {
    ...
    if (r && ((i = n.toolName) != null && i.startsWith("lat_")) && n.toolCallId) {
      ...
      r(
        !1,
        `${ie.COMPLETED_MARKER} ${n.toolName}`,
        a
      );
    }
  }
...
class bo {
  constructor(e, n = {}) {
    ...
    J(this, "handlers");
    J(this, "sendMessage");
    this.handlers = e, this.reasoningHandler = new To(e.onReasoningUpdate), this.toolHandler = new Eo(n, e.onReasoningUpdate);
  }
  handleMessage(e) {
    try {
    ...
    if (((n = e.data) == null ? void 0 : n.type) === $t.TOOL_RESULT && this.handlers.onReasoningUpdate) {
      ...
      if (r.toolCallId && r.toolName) {
        ...
        this.handlers.onReasoningUpdate(
          !1,
          `${ie.COMPLETED_MARKER} ${r.toolName}`,
          i
        );
      }
    }
  ...
  handleChatFinished(e) {
    var n, r;
    (r = (n = this.handlers).onSystemEvent) == null || r.call(n, an.chatCompleted(e.uuid));
  }
  ...
  handleChatError(e) {
    ...
    (r = (n = this.handlers).onSystemEvent) == null || r.call(
      n,
      an.chatError(e.error || "Unknown error")
    );
  }
  ...
  handleHeartbeatPing(e) {
    if (!this.sendMessage)
      return;
    const n = Ft.serializeHeartbeatPong(
      e.timestamp,
      e.pingTime
    );
    this.sendMessage(n);
  ...
  handleError(e) {
    ...
    (r = (n = this.handlers).onSystemEvent) == null || r.call(
      n,
      an.chatError(e.error || "Unknown WebSocket error")
    );
  }
  updateClientTools(e) {
...
class Lo {
  constructor() {
    ...
    this.config = {
      ...So
    }, this.connectionState = new ko(), this.wsManager = new xo(this.config, this.connectionState), this.messageHandler = new bo({}), this.setupWebSocketHandlers();
  }
  ...
  handleAuthenticationFailure(e) {
    ...
    (n == null ? void 0 : n.code) === "TICKET_INVALID" || (n == null ? void 0 : n.code) === "TICKET_EXPIRED" ? this.refreshTicketAndReconnect().catch((i) => {
      var s;
      ln(i, "TicketRefresh").isRetryable, (s = this.initReject) == null || s.call(this, i);
  ...
  sendToolConfiguration() {
    const e = Ft.serializeConfigureTools(
      this.toolSchemas,
      this.contextHelpers
    );
  ...
  async onTriggerMessage(e) {
    ...
    if (!this.connectionState.isConnected || n !== "OPEN") {
      const c = "Connection lost. Please check your internet connection and try again.";
      throw new Error(c);
    }
    const { message: i, media: a, providerResId: s, mcpHeaders: o } = e;
    try {
      this.messageHandler.clearProcessedToolCalls();
      const c = Ft.serializeChatMessage({
        content: i,
        media: a,
        providerResId: s,
        mcpHeaders: o
      });
      this.wsManager.send(c);
    } catch (c) {
      throw this.wsManager.getWebSocketState() !== "OPEN" ? new Error("Connection lost during message send. Please try again.") : c;
    }
  }
  disconnect() {
    var e, n;
  ...
  updateContextHelpers(e) {
    ...
    const n = Ft.serializeUpdateContextHelpers(
      this.contextHelpers
    );
  ...
  addClientTools(e, n) {
    ...
    const r = Ft.serializeUpdateTools(this.toolSchemas);
...
function Oo({
  ...
  const [d, C] = we(
    ...
    He.DISCONNECTED
  ), [T, b] = we(0), [v, O] = we(!0), A = de(null), E = de(o), U = de(c), B = de(u), G = de(l), Z = de(h), se = de(t), D = de(e), M = de(n), N = de(r), z = de(i), q = de(a), te = de(a);
  ke(() => {
    JSON.stringify(a) !== JSON.stringify(q.current) && (q.current = a, te.current = a);
  }, [a]);
  const ge = de(s), Se = de(
    s
  );
  ke(() => {
    JSON.stringify(s) !== JSON.stringify(ge.current) && (ge.current = s, Se.current = s, A.current && s && A.current.updateContextHelpers(s));
  }, [s]), ke(() => {
    E.current = o, U.current = c, B.current = u, G.current = l, Z.current = h, se.current = t, D.current = e, M.current = n, N.current = r, z.current = i;
  }, [
    o,
    c,
    u,
  ...
  const w = ze(() => {
    const oe = te.current;
    return oe && oe.length > 0 ? oe.map(({ execute: re, ...Q }) => Q) : [];
  }, [te.current]), Y = ze(() => {
    if (a && a.length > 0) {
      const oe = {};
      return a.forEach((re) => {
        oe[re.name] = re.execute;
      }), oe;
    }
    return {};
  }, [a]);
  ke(() => {
    A.current && Object.keys(Y).length > 0 && A.current.updateClientTools(Y);
  }, [Y]);
  const W = de(), y = ae(async () => {
    var oe;
    try {
      if (!navigator.onLine)
        throw I(He.DISCONNECTED), O(!1), new Error("No internet connection. Please check your network and try again.");
      if (I(He.CONNECTING), !se.current)
      ...
      await re.onInit({
        ...
        userMpAuthToken: se.current,
        chatServerUrl: D.current,
        chatServerKey: M.current,
        entityId: N.current,
        entityType: (oe = z.current) == null ? void 0 : oe.toString(),
        // Tools configuration
        toolSchemas: w,
        clientTools: Y,
        contextHelpers: Q,
      ...
      const Q = ln(re, "WebSocketConnection");
      I(He.DISCONNECTED), Q.isRetryable ? setTimeout(() => {
        var Te;
        (A.current === null || !A.current.getConnectionStatus().connected) && ((Te = W.current) == null || Te.call(W));
      }, 2e3) : O(!1);
    }
  }, [
    w,
    ...
    A.current && (A.current.disconnect(), A.current = null), C(null), I(He.DISCONNECTED);
  }, []);
  W.current = y;
  const $ = de(!1);
  return ke(() => ($.current || ($.current = !0, y()), () => {
    ue();
  }), []), ke(() => {
    const oe = setInterval(() => {
      if (A.current) {
        const re = A.current.getConnectionStatus();
        if (v && k === He.CONNECTING)
          return;
        re.connected && k !== He.CONNECTED ? I(He.CONNECTED) : re.isReconnecting && k !== He.RECONNECTING ? I(He.RECONNECTING) : !re.connected && !re.isReconnecting && k !== He.DISCONNECTED && I(He.DISCONNECTED), b(re.reconnectAttempts);
      }
    }, 1e3);
    ...
    connectionState: k,
    reconnectAttempts: T,
    isInitialConnection: v,
    connectChatClient: y,
    disconnectChatClient: ue
  };
}
...
const Un = Qe(Array.prototype.forEach), Ho = Qe(Array.prototype.lastIndexOf), Mi = Qe(Array.prototype.pop), fn = Qe(Array.prototype.push), zo = Qe(Array.prototype.splice), Kn = Qe(String.prototype.toLowerCase), dr = Qe(String.prototype.toString), pr = Qe(String.prototype.match), gn = Qe(String.prototype.replace), Uo = Qe(String.prototype.indexOf), Bo = Qe(String.prototype.trim), Ct = Qe(Object.prototype.hasOwnProperty), Xe = Qe(RegExp.prototype.test), mn = $o(TypeError);
function Qe(t) {
  return function(e) {
...
function Dt(t) {
  const e = Fr(null);
  for (const [n, r] of es(t))
    Ct(t, n) && (Array.isArray(r) ? e[n] = jo(r) : r && typeof r == "object" && r.constructor === Object ? e[n] = Dt(r) : e[n] = r);
  return e;
}
function yn(t, e) {
  for (; t !== null; ) {
...
function rs() {
  let t = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : el();
  const e = (ee) => rs(ee);
  ...
  const r = n, i = r.currentScript, {
    ...
    DOMParser: g,
    trustedTypes: d
  } = t, C = c.prototype, k = yn(C, "cloneNode"), I = yn(C, "remove"), T = yn(C, "nextSibling"), b = yn(C, "childNodes"), v = yn(C, "parentNode");
  ...
  let se = Di();
  e.isSupported = typeof es == "function" && typeof v == "function" && E && E.createHTMLDocument !== void 0;
  const {
    MUSTACHE_EXPR: D,
    ERB_EXPR: M,
    TMPLIT_EXPR: N,
  ...
  const W = fe({}, [...Ni, ...fr, ...gr, ...mr, ...Ai]);
  let y = null;
  const ue = fe({}, [...Li, ...yr, ...Oi, ...Bn]);
  let $ = Object.seal(Fr(null, {
    tagNameCheck: {
      writable: !0,
      configurable: !1,
      enumerable: !0,
  ...
  let Te = !0, me = !0, ye = !1, Ie = !0, Ue = !1, at = !0, st = !1, Rt = !1, It = !1, yt = !1, Mt = !1, Nt = !1, Gt = !0, Wt = !1;
  const Jt = "user-content-";
  let At = !0, ot = !1, _ = {}, R = null;
  const V = fe({}, ["annotation-xml", "audio", "colgroup", "desc", "foreignobject", "head", "iframe", "math", "mi", "mn", "mo", "ms", "mtext", "noembed", "noframes", "noscript", "plaintext", "script", "style", "svg", "template", "thead", "title", "video", "xmp"]);
  let he = null;
  const ne = fe({}, ["audio", "video", "img", "source", "image", "track"]);
  let Ee = null;
  const lt = fe({}, ["alt", "class", "for", "id", "label", "name", "pattern", "placeholder", "role", "summary", "title", "value", "style", "xmlns"]), Le = "http://www.w3.org/1998/Math/MathML", ct = "http://www.w3.org/2000/svg", Be = "http://www.w3.org/1999/xhtml";
  let Ae = Be, Ve = !1, $e = null;
  ...
  const Dn = n.createElement("form"), pn = function(S) {
    ...
    if (!(Lt && Lt === S)) {
      ...
      On.indexOf(S.PARSER_MEDIA_TYPE) === -1 ? Pn : S.PARSER_MEDIA_TYPE, Oe = zt === "application/xhtml+xml" ? dr : Kn, Y = Ct(S, "ALLOWED_TAGS") ? fe({}, S.ALLOWED_TAGS, Oe) : W, y = Ct(S, "ALLOWED_ATTR") ? fe({}, S.ALLOWED_ATTR, Oe) : ue, $e = Ct(S, "ALLOWED_NAMESPACES") ? fe({}, S.ALLOWED_NAMESPACES, dr) : dn, Ee = Ct(S, "ADD_URI_SAFE_ATTR") ? fe(Dt(lt), S.ADD_URI_SAFE_ATTR, Oe) : lt, he = Ct(S, "ADD_DATA_URI_TAGS") ? fe(Dt(ne), S.ADD_DATA_URI_TAGS, Oe) : ne, R = Ct(S, "FORBID_CONTENTS") ? fe({}, S.FORBID_CONTENTS, Oe) : V, oe = Ct(S, "FORBID_TAGS") ? fe({}, S.FORBID_TAGS, Oe) : Dt({}), re = Ct(S, "FORBID_ATTR") ? fe({}, S.FORBID_ATTR, Oe) : Dt({}), _ = Ct(S, "USE_PROFILES") ? S.USE_PROFILES : !1, Te = S.ALLOW_ARIA_ATTR !== !1, me = S.ALLOW_DATA_ATTR !== !1, ye = S.ALLOW_UNKNOWN_PROTOCOLS || !1, Ie = S.ALLOW_SELF_CLOSE_IN_ATTR !== !1, Ue = S.SAFE_FOR_TEMPLATES || !1, at = S.SAFE_FOR_XML !== !1, st = S.WHOLE_DOCUMENT || !1, yt = S.RETURN_DOM || !1, Mt = S.RETURN_DOM_FRAGMENT || !1, Nt = S.RETURN_TRUSTED_TYPE || !1, It = S.FORCE_BODY || !1, Gt = S.SANITIZE_DOM !== !1, Wt = S.SANITIZE_NAMED_PROPS || !1, At = S.KEEP_CONTENT !== !1, ot = S.IN_PLACE || !1, w = S.ALLOWED_URI_REGEXP || ts, Ae = S.NAMESPACE || Be, qt = S.MATHML_TEXT_INTEGRATION_POINTS || qt, Qt = S.HTML_INTEGRATION_POINTS || Qt, $ = S.CUSTOM_ELEMENT_HANDLING || {}, S.CUSTOM_ELEMENT_HANDLING && pn(S.CUSTOM_ELEMENT_HANDLING.tagNameCheck) && ($.tagNameCheck = S.CUSTOM_ELEMENT_HANDLING.tagNameCheck), S.CUSTOM_ELEMENT_HANDLING && pn(S.CUSTOM_ELEMENT_HANDLING.attributeNameCheck) && ($.attributeNameCheck = S.CUSTOM_ELEMENT_HANDLING.attributeNameCheck), S.CUSTOM_ELEMENT_HANDLING && typeof S.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements == "boolean" && ($.allowCustomizedBuiltInElements = S.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements), Ue && (me = !1), Mt && (yt = !0), _ && (Y = fe({}, Ai), y = [], _.html === !0 && (fe(Y, Ni), fe(y, Li)), _.svg === !0 && (fe(Y, fr), fe(y, yr), fe(y, Bn)), _.svgFilters === !0 && (fe(Y, gr), fe(y, yr), fe(y, Bn)), _.mathMl === !0 && (fe(Y, mr), fe(y, Oi), fe(y, Bn))), S.ADD_TAGS && (typeof S.ADD_TAGS == "function" ? Q.tagCheck = S.ADD_TAGS : (Y === W && (Y = Dt(Y)), fe(Y, S.ADD_TAGS, Oe))), S.ADD_ATTR && (typeof S.ADD_ATTR == "function" ? Q.attributeCheck = S.ADD_ATTR : (y === ue && (y = Dt(y)), fe(y, S.ADD_ATTR, Oe))), S.ADD_URI_SAFE_ATTR && fe(Ee, S.ADD_URI_SAFE_ATTR, Oe), S.FORBID_CONTENTS && (R === V && (R = Dt(R)), fe(R, S.FORBID_CONTENTS, Oe)), At && (Y["#text"] = !0), st && fe(Y, ["html", "head", "body"]), Y.table && (fe(Y, ["tbody"]), delete oe.tbody), S.TRUSTED_TYPES_POLICY) {
        ...
        O = S.TRUSTED_TYPES_POLICY, A = O.createHTML("");
      } else
        O === void 0 && (O = tl(d, i)), O !== null && typeof A == "string" && (A = O.createHTML(""));
      Je && Je(S), Lt = S;
    }
  }, Fn = fe({}, [...fr, ...gr, ...Vo]), ur = fe({}, [...mr, ...Go]), Hn = function(S) {
    let H = v(S);
    (!H || !H.tagName) && (H = {
      namespaceURI: Ae,
      tagName: "template"
    });
    const X = Kn(S.tagName), Me = Kn(H.tagName);
    return $e[S.namespaceURI] ? S.namespaceURI === ct ? H.namespaceURI === Be ? X === "svg" : H.namespaceURI === Le ? X === "svg" && (Me === "annotation-xml" || qt[Me]) : !!Fn[X] : S.namespaceURI === Le ? H.namespaceURI === Be ? X === "math" : H.namespaceURI === ct ? X === "math" && Qt[Me] : !!ur[X] : S.namespaceURI === Be ? H.namespaceURI === ct && !Qt[Me] || H.namespaceURI === Le && !qt[Me] ? !1 : !ur[X] && (cr[X] || !Fn[X]) : !!(zt === "application/xhtml+xml" && $e[S.namespaceURI]) : !1;
  }, ut = function(S) {
    fn(e.removed, {
      element: S
    });
    ...
    const Me = O ? O.createHTML(S) : S;
    if (Ae === Be)
      try {
        H = new g().parseFromString(Me, zt);
      } catch {
      }
    if (!H || !H.documentElement) {
      H = E.createDocument(Ae, "template", null);
      try {
        H.documentElement.innerHTML = Ve ? A : Me;
      } catch {
      }
    }
    const We = H.body || H.documentElement;
    return S && X && We.insertBefore(n.createTextNode(X), We.childNodes[0] || null), Ae === Be ? G.call(H, st ? "html" : "body")[0] : st ? H.documentElement : We;
  }, Ye = function(S) {
    return U.call(
      S.ownerDocument || S,
      S,
  ...
  const Ot = function(S) {
    let H = null;
    if (Re(se.beforeSanitizeElements, S, null), Pe(S))
      return ut(S), !0;
    const X = Oe(S.nodeName);
    if (Re(se.uponSanitizeElement, S, {
      tagName: X,
      allowedTags: Y
    }), at && S.hasChildNodes() && !Ne(S.firstElementChild) && Xe(/<[/\w!]/g, S.innerHTML) && Xe(/<[/\w!]/g, S.textContent) || S.nodeType === Cn.progressingInstruction || at && S.nodeType === Cn.comment && Xe(/<[/\w]/g, S.data))
      return ut(S), !0;
    if (!(Q.tagCheck instanceof Function && Q.tagCheck(X)) && (!Y[X] || oe[X])) {
      if (!oe[X] && Ei(X) && ($.tagNameCheck instanceof RegExp && Xe($.tagNameCheck, X) || $.tagNameCheck instanceof Function && $.tagNameCheck(X)))
        return !1;
      if (At && !R[X]) {
        const Me = v(S) || S.parentNode, We = b(S) || S.childNodes;
        if (We && Me) {
          const De = We.length;
          for (let tt = De - 1; tt >= 0; --tt) {
            const Pt = k(We[tt], !0);
            Pt.__removalCount = (S.__removalCount || 0) + 1, Me.insertBefore(Pt, T(S));
          }
        }
      }
      return ut(S), !0;
    }
    return S instanceof c && !Hn(S) || (X === "noscript" || X === "noembed" || X === "noframes") && Xe(/<\/no(script|embed|frames)/i, S.innerHTML) ? (ut(S), !0) : (Ue && S.nodeType === Cn.text && (H = S.textContent, Un([D, M, N], (Me) => {
      H = gn(H, Me, " ");
    }), S.textContent !== H && (fn(e.removed, {
      element: S.cloneNode()
    }), S.textContent = H)), Re(se.afterSanitizeElements, S, null), !1);
  }, Ut = function(S, H, X) {
    if (Gt && (H === "id" || H === "name") && (X in n || X in Dn))
      return !1;
    if (!(me && !re[H] && Xe(z, H))) {
      if (!(Te && Xe(q, H))) {
        if (!(Q.attributeCheck instanceof Function && Q.attributeCheck(H, S))) {
          if (!y[H] || re[H]) {
            if (
              // First condition does a very basic check if a) it's basically a valid custom element tagname AND
              // b) if the tagName passes whatever the user has configured for CUSTOM_ELEMENT_HANDLING.tagNameCheck
              // and c) if the attribute name passes whatever the user has configured for CUSTOM_ELEMENT_HANDLING.attributeNameCheck
              !(Ei(S) && ($.tagNameCheck instanceof RegExp && Xe($.tagNameCheck, S) || $.tagNameCheck instanceof Function && $.tagNameCheck(S)) && ($.attributeNameCheck instanceof RegExp && Xe($.attributeNameCheck, H) || $.attributeNameCheck instanceof Function && $.attributeNameCheck(H, S)) || // Alternative, second condition checks if it's an `is`-attribute, AND
              // the value passes whatever the user has configured for CUSTOM_ELEMENT_HANDLING.tagNameCheck
              H === "is" && $.allowCustomizedBuiltInElements && ($.tagNameCheck instanceof RegExp && Xe($.tagNameCheck, X) || $.tagNameCheck instanceof Function && $.tagNameCheck(X)))
            ) return !1;
          } else if (!Ee[H]) {
            if (!Xe(w, gn(X, ge, ""))) {
              if (!((H === "src" || H === "xlink:href" || H === "href") && S !== "script" && Uo(X, "data:") === 0 && he[S])) {
                if (!(ye && !Xe(te, gn(X, ge, "")))) {
                  if (X)
                    return !1;
                }
    ...
    return S !== "annotation-xml" && pr(S, Se);
  }, bi = function(S) {
    Re(se.beforeSanitizeAttributes, S, null);
    const {
      attributes: H
    } = S;
    if (!H || Pe(S))
    ...
    for (; Me--; ) {
      ...
      let Ge = De === "value" ? hr : Bo(hr);
      if (X.attrName = tn, X.attrValue = Ge, X.keepAttr = !0, X.forceKeepAttr = void 0, Re(se.uponSanitizeAttribute, S, X), Ge = X.attrValue, Wt && (tn === "id" || tn === "name") && (L(De, S), Ge = Jt + Ge), at && Xe(/((--!?|])>)|<\/(style|title|textarea)/i, Ge)) {
        L(De, S);
        continue;
      }
      if (tn === "attributename" && pr(Ge, "href")) {
        L(De, S);
        continue;
      }
      if (X.forceKeepAttr)
      ...
      if (O && typeof d == "object" && typeof d.getAttributeType == "function" && !tt)
        switch (d.getAttributeType(_i, tn)) {
          case "TrustedHTML": {
            Ge = O.createHTML(Ge);
            break;
          }
          case "TrustedScriptURL": {
            Ge = O.createScriptURL(Ge);
            break;
          }
        }
      if (Ge !== hr)
        try {
          tt ? S.setAttributeNS(tt, De, Ge) : S.setAttribute(De, Ge), Pe(S) ? ut(S) : Mi(e.removed);
        } catch {
          L(De, S);
        }
    }
    Re(se.afterSanitizeAttributes, S, null);
  }, go = function ee(S) {
    let H = null;
    const X = Ye(S);
    for (Re(se.beforeSanitizeShadowDOM, S, null); H = X.nextNode(); )
      Re(se.uponSanitizeShadowNode, H, null), Ot(H), bi(H), H.content instanceof a && ee(H.content);
    Re(se.afterSanitizeShadowDOM, S, null);
  };
  return e.sanitize = function(ee) {
    let S = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}, H = null, X = null, Me = null, We = null;
    if (Ve = !ee, Ve && (ee = "<!-->"), typeof ee != "string" && !Ne(ee))
    ...
    if (!e.isSupported)
      return ee;
    if (Rt || en(S), e.removed = [], typeof ee == "string" && (ot = !1), ot) {
      if (ee.nodeName) {
        const Pt = Oe(ee.nodeName);
        if (!Y[Pt] || oe[Pt])
          throw mn("root node is forbidden and cannot be sanitized in-place");
      }
    } else if (ee instanceof o)
      H = xe("<!---->"), X = H.ownerDocument.importNode(ee, !0), X.nodeType === Cn.element && X.nodeName === "BODY" || X.nodeName === "HTML" ? H = X : H.appendChild(X);
    else {
      if (!yt && !Ue && !st && // eslint-disable-next-line unicorn/prefer-includes
      ee.indexOf("<") === -1)
        return O && Nt ? O.createHTML(ee) : ee;
      if (H = xe(ee), !H)
        return yt ? null : Nt ? A : "";
    }
    H && It && ut(H.firstChild);
    const De = Ye(ot ? ee : H);
    for (; Me = De.nextNode(); )
      Ot(Me), bi(Me), Me.content instanceof a && go(Me.content);
    if (ot)
      return ee;
    if (yt) {
      if (Mt)
        for (We = B.call(H.ownerDocument); H.firstChild; )
          We.appendChild(H.firstChild);
      else
        We = H;
      return (y.shadowroot || y.shadowrootmode) && (We = Z.call(r, We, !0)), We;
    }
    let tt = st ? H.outerHTML : H.innerHTML;
    return st && Y["!doctype"] && H.ownerDocument && H.ownerDocument.doctype && H.ownerDocument.doctype.name && Xe(ns, H.ownerDocument.doctype.name) && (tt = "<!DOCTYPE " + H.ownerDocument.doctype.name + `>
` + tt), Ue && Un([D, M, N], (Pt) => {
      tt = gn(tt, Pt, " ");
    }), O && Nt ? O.createHTML(tt) : tt;
    ...
    if (S !== void 0) {
      const H = Ho(se[ee], S);
      return H === -1 ? void 0 : zo(se[ee], H, 1)[0];
    }
    return Mi(se[ee]);
...
function rl(t, e = "userMessage") {
  if (typeof t != "string")
    return console.warn("sanitizeInput received non-string input:", typeof t), "";
  if (!t.trim())
    return "";
  try {
    const n = nl[e], r = is.sanitize(t, n);
    return as(r) ? (console.warn("Suspicious content detected and removed:", t), r.replace(/javascript:/gi, "").replace(/data:/gi, "")) : r;
  } catch (n) {
...
function si(t, e = !1) {
  ...
  return rl(r, e ? "assistantMessage" : "userMessage").replace(new RegExp(n, "g"), `
`);
}
function il(t) {
...
function Fi(t) {
  ...
  try {
    const e = new URL(t);
    return !(!["http:", "https:", "data:"].includes(e.protocol) || as(t));
  } catch {
    return !1;
  }
}
function al() {
  is.addHook("beforeSanitizeAttributes", (t) => {
...
const ul = (t) => {
  const e = ol(t), n = (r) => cl(e, r);
  return Object.assign(n, e), n;
}, hl = (t) => ul, zi = { BASE_URL: "/", DEV: !0, MODE: "production", PROD: !1, SSR: !1, VITE_APP_TOLGEE_API_KEY: "tgpak_ge4v6ytcm5xtozdlobzxk4twgyzwqntjoe3xi4bwozygu5q", VITE_APP_TOLGEE_API_URL: "https://tolgee-translation.oddleapp.com", VITE_APP_TOLGEE_PROJECT_ID: "19", VITE_USER_NODE_ENV: "development" }, vn = /* @__PURE__ */ new Map(), $n = (t) => {
  const e = vn.get(t);
  ...
  return vn.set(n.name, i), { type: "tracked", store: t, ...i };
}, pl = (t, e) => {
  if (e === void 0) return;
  const n = vn.get(t);
  n && (delete n.stores[e], Object.keys(n.stores).length === 0 && vn.delete(t));
}, fl = (t) => {
  var e, n;
  if (!t) return;
  const r = t.split(`
`), i = r.findIndex(
  ...
  if (!u)
    return t(n, r, i);
  const { connection: l, ...h } = dl(o, u, c);
  let g = !0;
  i.setState = (k, I, T) => {
    const b = n(k, I);
    if (!g) return b;
    const v = T === void 0 ? {
      type: s || fl(new Error().stack) || "anonymous"
    } : typeof T == "string" ? { type: T } : T;
    return o === void 0 ? (l == null || l.send(v, r()), b) : (l == null || l.send(
      {
        ...v,
        type: `${o}/${v.type}`
      },
    ...
    cleanup: () => {
      l && typeof l.unsubscribe == "function" && l.unsubscribe(), pl(c.name, o);
    }
  };
  const d = (...k) => {
    const I = g;
  ...
  if (h.type === "untracked" ? l == null || l.init(C) : (h.stores[h.store] = i, l == null || l.init(
    Object.fromEntries(
      Object.entries(h.stores).map(([k, I]) => [
        k,
        k === h.store ? C : I.getState()
      ])
    )
    ...
    const I = i.dispatch;
    i.dispatch = (...T) => {
      (zi ? "production" : void 0) !== "production" && T[0].type === "__setState" && !k && (console.warn(
        '[zustand devtools middleware] "__setState" action type is reserved to set state from the devtools. Avoid using it.'
      ), k = !0), I(...T);
    };
  }
  return l.subscribe((k) => {
    var I;
    switch (k.type) {
      case "ACTION":
      ...
      case "DISPATCH":
        switch (k.payload.type) {
          ...
          case "IMPORT_STATE": {
            const { nextLiftedState: T } = k.payload, b = (I = T.computedStates.slice(-1)[0]) == null ? void 0 : I.state;
            if (!b) return;
...
function kl() {
  const t = le((b) => b.isStreaming), e = le((b) => b.setIsStreaming), n = le((b) => b.isThinking), r = le((b) => b.setIsThinking), i = le((b) => b.streamingContent), a = le((b) => b.setStreamingContent), s = le((b) => b.isHandlingTool), o = le((b) => b.setIsHandlingTool), c = le((b) => b.startStreaming), u = le((b) => b.stopStreaming), l = le((b) => b.clearStreamingBuffers), h = le((b) => b.resetToolHandling), g = de(""), d = ze(() => ({
    get current() {
      return le.getState().currentAssistantMessageId;
    },
    set current(b) {
      le.getState().setCurrentAssistantMessageId(b);
    }
  }), []), C = ae((b) => {
    b ? c(b) : (e(!0), r(!0), a("")), g.current = "";
  }, [c, e, r, a]), k = ae(() => {
    u(), g.current = "";
  }, [u]), I = ae(() => {
    h();
...
const ce = (t) => typeof t == "string", wn = () => {
  let t, e;
  const n = new Promise((r, i) => {
    t = r, e = i;
  });
  return n.resolve = t, n.reject = e, n;
}, Ui = (t) => t == null ? "" : "" + t, Tl = (t, e, n) => {
  t.forEach((r) => {
    e[r] && (n[r] = e[r]);
  });
}, El = /###/g, Bi = (t) => t && t.indexOf("###") > -1 ? t.replace(El, ".") : t, $i = (t) => !t || ce(t), kn = (t, e, n) => {
  ...
  let a = e[e.length - 1], s = e.slice(0, e.length - 1), o = kn(t, s, Object);
  for (; o.obj === void 0 && s.length; )
    a = `${s[s.length - 1]}.${a}`, s = s.slice(0, s.length - 1), o = kn(t, s, Object), o != null && o.obj && typeof o.obj[`${o.k}.${a}`] < "u" && (o.obj = void 0);
  o.obj[`${o.k}.${a}`] = n;
}, bl = (t, e, n, r) => {
  const {
    obj: i,
  ...
  for (const r in e)
    r !== "__proto__" && r !== "constructor" && (r in t ? ce(t[r]) || t[r] instanceof String || ce(e[r]) || e[r] instanceof String ? n && (t[r] = e[r]) : ss(t[r], e[r], n) : t[r] = e[r]);
  return t;
}, nn = (t) => t.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&");
var vl = {
  "&": "&amp;",
...
const Rl = (t) => ce(t) ? t.replace(/[&<>"'\/]/g, (e) => vl[e]) : t;
class Il {
  constructor(e) {
...
const Ml = [" ", ",", "?", "!", ";"], Nl = new Il(20), Al = (t, e, n) => {
  e = e || "", n = n || "";
  const r = Ml.filter((s) => e.indexOf(s) < 0 && n.indexOf(s) < 0);
  if (r.length === 0) return !0;
  const i = Nl.getRegExp(`(${r.map((s) => s === "?" ? "\\?" : s).join("|")})`);
  let a = !i.test(t);
  if (!a) {
    const s = t.indexOf(n);
  ...
  return i;
}, Rn = (t) => t == null ? void 0 : t.replace("_", "-"), Ll = {
  type: "logger",
  log(t) {
    this.output("log", t);
  },
...
class Qn {
  ...
  init(e, n = {}) {
    this.prefix = n.prefix || "i18next:", this.logger = e || Ll, this.options = n, this.debug = n.debug;
  }
  log(...e) {
...
class Vi extends ir {
  ...
  getResource(e, n, r, i = {}) {
    ...
    return !c && !n && !r && e.indexOf(".") > -1 && (e = o[0], n = o[1], r = o.slice(2).join(".")), c || !s || !ce(r) ? c : Ur((l = (u = this.data) == null ? void 0 : u[e]) == null ? void 0 : l[n], r, a);
  }
  ...
  addResourceBundle(e, n, r, i, a, s = {
    ...
    s.skipCopy || (r = JSON.parse(JSON.stringify(r))), i ? ss(c, r, a) : c = {
      ...c,
      ...r
...
const ls = Symbol("i18next/PATH_KEY");
function Ol() {
  const t = [], e = /* @__PURE__ */ Object.create(null);
  let n;
  return e.get = (r, i) => {
    var a;
    return (a = n == null ? void 0 : n.revoke) == null || a.call(n), i === ls ? t : (t.push(i), n = Proxy.revocable(r, e), n.proxy);
...
const Gi = {}, wr = (t) => !ce(t) && typeof t != "boolean" && typeof t != "number";
class er extends ir {
  constructor(e, n = {}) {
    super(), Tl(["resourceStore", "languageUtils", "pluralResolver", "interpolator", "backendConnector", "i18nFormat", "utils"], e, this), this.options = n, this.options.keySeparator === void 0 && (this.options.keySeparator = "."), this.logger = _t.create("translator");
  }
  changeLanguage(e) {
    e && (this.language = e);
  }
  ...
  extractFromKey(e, n) {
    ...
    const s = r && e.indexOf(r) > -1, o = !this.options.userDefinedKeySeparator && !n.keySeparator && !this.options.userDefinedNsSeparator && !n.nsSeparator && !Al(e, r, i);
    if (s && !o) {
  ...
  translate(e, n, r) {
    ...
    const k = (d == null ? void 0 : d.usedKey) || o, I = (d == null ? void 0 : d.exactUsedKey) || o, T = ["[object Number]", "[object Function]", "[object RegExp]"], b = i.joinArrays !== void 0 ? i.joinArrays : this.options.joinArrays, v = !this.i18nFormat || this.i18nFormat.handleAsObject, O = i.count !== void 0 && !ce(i.count), A = er.hasDefaultValue(i), E = O ? this.pluralResolver.getSuffix(h, i.count, i) : "", U = i.ordinal && O ? this.pluralResolver.getSuffix(h, i.count, {
      ordinal: !1
    }) : "", B = O && !i.ordinal && i.count === 0, G = B && i[`defaultValue${this.options.pluralSeparator}zero`] || i[`defaultValue${E}`] || i[`defaultValue${U}`] || i.defaultValue;
    let Z = C;
    v && !C && A && (Z = G);
    const se = wr(Z), D = Object.prototype.toString.apply(Z);
    if (v && Z && se && T.indexOf(D) < 0 && !(ce(b) && Array.isArray(Z))) {
      if (!i.returnObjects && !this.options.returnObjects) {
        this.options.returnedObjectHandler || this.logger.warn("accessing an object - but returnObjects options is not enabled!");
        const M = this.options.returnedObjectHandler ? this.options.returnedObjectHandler(k, Z, {
          ...i,
          ns: c
        }) : `key '${o} (${this.language})' returned an object instead of string.`;
        return a ? (d.res = M, d.usedParams = this.getUsedParamsDetails(i), d) : M;
      }
      if (s) {
        const M = Array.isArray(Z), N = M ? [] : {}, z = M ? I : k;
        for (const q in Z)
          if (Object.prototype.hasOwnProperty.call(Z, q)) {
            const te = `${z}${s}${q}`;
            A && !C ? N[q] = this.translate(te, {
              ...i,
              defaultValue: wr(G) ? G[q] : void 0,
              joinArrays: !1,
              ns: c
            }) : N[q] = this.translate(te, {
              ...i,
              joinArrays: !1,
              ns: c
            }), N[q] === te && (N[q] = Z[q]);
    ...
    } else if (v && ce(b) && Array.isArray(C))
      C = C.join(b), C && (C = this.extendTranslation(C, e, i, r));
    else {
      let M = !1, N = !1;
      !this.isValidLookup(C) && A && (M = !0, C = G), this.isValidLookup(C) || (N = !0, C = o);
      const q = (i.missingKeyNoValueFallbackToKey || this.options.missingKeyNoValueFallbackToKey) && N ? void 0 : C, te = A && G !== C && this.options.updateMissing;
      if (N || M || te) {
        if (this.logger.log(te ? "updateKey" : "missingKey", h, u, o, te ? G : C), s) {
        ...
        const w = (Y, W, y) => {
          var $;
          const ue = A && y !== C ? y : q;
          this.options.missingKeyHandler ? this.options.missingKeyHandler(Y, u, W, ue, te, i) : ($ = this.backendConnector) != null && $.saveMissing && this.backendConnector.saveMissing(Y, u, W, ue, te, i), this.emit("missingKey", Y, u, W, C);
        };
        this.options.saveMissing && (this.options.saveMissingPlurals && O ? ge.forEach((Y) => {
          const W = this.pluralResolver.getSuffixes(Y, i);
          B && i[`defaultValue${this.options.pluralSeparator}zero`] && W.indexOf(`${this.options.pluralSeparator}zero`) < 0 && W.push(`${this.options.pluralSeparator}zero`), W.forEach((y) => {
            w([Y], o + y, i[`defaultValue${y}`] || G);
          });
        }) : w(ge, o, G));
      }
      C = this.extendTranslation(C, e, i, d, r), N && C === o && this.options.appendNamespaceToMissingKey && (C = `${u}${l}${o}`), (N || M) && this.options.parseMissingKeyHandler && (C = this.options.parseMissingKeyHandler(this.options.appendNamespaceToMissingKey ? `${u}${l}${o}` : o, M ? C : void 0, i));
    }
  ...
  extendTranslation(e, n, r, i, a) {
    ...
    if ((c = this.i18nFormat) != null && c.parse)
      ...
      const l = ce(e) && (((u = r == null ? void 0 : r.interpolation) == null ? void 0 : u.skipOnVariables) !== void 0 ? r.interpolation.skipOnVariables : this.options.interpolation.skipOnVariables);
      let h;
      if (l) {
      ...
      if (this.options.interpolation.defaultVariables && (g = {
        ...this.options.interpolation.defaultVariables,
        ...g
      }), e = this.interpolator.interpolate(e, g, r.lng || this.language || i.usedLng, r), l) {
    ...
    return e != null && (o != null && o.length) && r.applyPostProcessor !== !1 && (e = os.handle(o, e, n, this.options && this.options.postProcessPassResolved ? {
      i18nResolved: {
        ...i,
  ...
  resolve(e, n = {}) {
    let r, i, a, s, o;
    return ce(e) && (e = [e]), e.forEach((c) => {
      if (this.isValidLookup(r)) return;
      const u = this.extractFromKey(c, n), l = u.key;
      i = l;
      let h = u.namespaces;
      this.options.fallbackNS && (h = h.concat(this.options.fallbackNS));
      const g = n.count !== void 0 && !ce(n.count), d = g && !n.ordinal && n.count === 0, C = n.context !== void 0 && (ce(n.context) || typeof n.context == "number") && n.context !== "", k = n.lngs ? n.lngs : this.languageUtils.toResolveHierarchy(n.lng || this.language, n.fallbackLng);
      h.forEach((I) => {
        var T, b;
        this.isValidLookup(r) || (o = I, !Gi[`${k[0]}-${I}`] && ((T = this.utils) != null && T.hasLoadedNamespace) && !((b = this.utils) != null && b.hasLoadedNamespace(o)) && (Gi[`${k[0]}-${I}`] = !0, this.logger.warn(`key "${i}" for languages "${k.join(", ")}" won't get resolved as namespace "${o}" was not yet loaded`, "This means something IS WRONG in your setup. You access the t function before i18next.init / i18next.loadNamespace / i18next.changeLanguage was done. Wait for the callback or Promise to resolve before accessing it!!!")), k.forEach((v) => {
          var E;
          if (this.isValidLookup(r)) return;
          s = v;
          const O = [l];
          if ((E = this.i18nFormat) != null && E.addLookupKeys)
            this.i18nFormat.addLookupKeys(O, l, v, I, n);
          else {
            let U;
            g && (U = this.pluralResolver.getSuffix(v, n.count, n));
            const B = `${this.options.pluralSeparator}zero`, G = `${this.options.pluralSeparator}ordinal${this.options.pluralSeparator}`;
            if (g && (n.ordinal && U.indexOf(G) === 0 && O.push(l + U.replace(G, this.options.pluralSeparator)), O.push(l + U), d && O.push(l + B)), C) {
              const Z = `${l}${this.options.contextSeparator || "_"}${n.context}`;
              O.push(Z), g && (n.ordinal && U.indexOf(G) === 0 && O.push(Z + U.replace(G, this.options.pluralSeparator)), O.push(Z + U), d && O.push(Z + B));
            }
          }
          let A;
          for (; A = O.pop(); )
            this.isValidLookup(r) || (a = A, r = this.getResource(v, I, A, n));
        }));
      });
    }), {
      res: r,
...
class Wi {
  constructor(e) {
    this.options = e, this.supportedLngs = this.options.supportedLngs || !1, this.logger = _t.create("languageUtils");
  }
  getScriptPartFromCode(e) {
    if (e = Rn(e), !e || e.indexOf("-") < 0) return null;
    const n = e.split("-");
    return n.length === 2 || (n.pop(), n[n.length - 1].toLowerCase() === "x") ? null : this.formatLanguageCode(n.join("-"));
  }
  getLanguagePartFromCode(e) {
    if (e = Rn(e), !e || e.indexOf("-") < 0) return e;
    const n = e.split("-");
...
class Pl {
  constructor(e, n = {}) {
    this.languageUtils = e, this.options = n, this.logger = _t.create("pluralResolver"), this.pluralRulesCache = {};
  }
  clearCache() {
    this.pluralRulesCache = {};
  }
  getRule(e, n = {}) {
    const r = Rn(e === "dev" ? "en" : e), i = n.ordinal ? "ordinal" : "cardinal", a = JSON.stringify({
      cleanedCode: r,
      type: i
    });
...
const Zi = (t, e, n, r = ".", i = !0) => {
  let a = _l(t, e, n);
  return !a && i && ce(n) && (a = Ur(t, n, r), a === void 0 && (a = Ur(e, n, r))), a;
}, Sr = (t) => t.replace(/\$/g, "$$$$");
class Yi {
  constructor(e = {}) {
    var n;
    this.logger = _t.create("interpolator"), this.options = e, this.format = ((n = e == null ? void 0 : e.interpolation) == null ? void 0 : n.format) || ((r) => r), this.init(e);
  }
  init(e = {}) {
    ...
    this.escape = n !== void 0 ? n : Rl, this.escapeValue = r !== void 0 ? r : !0, this.useRawValueToEscape = i !== void 0 ? i : !1, this.prefix = a ? nn(a) : s || "{{", this.suffix = o ? nn(o) : c || "}}", this.formatSeparator = u || ",", this.unescapePrefix = l ? "" : h || "-", this.unescapeSuffix = this.unescapePrefix ? "" : l || "", this.nestingPrefix = g ? nn(g) : d || nn("$t("), this.nestingSuffix = C ? nn(C) : k || nn(")"), this.nestingOptionsSeparator = I || ",", this.maxReplaces = T || 1e3, this.alwaysFormat = b !== void 0 ? b : !1, this.resetRegExp();
  }
  reset() {
  ...
  interpolate(e, n, r, i) {
    ...
    const c = this.options && this.options.interpolation && this.options.interpolation.defaultVariables || {}, u = (C) => {
      ...
      const k = C.split(this.formatSeparator), I = k.shift().trim(), T = k.join(this.formatSeparator).trim();
    ...
    return [{
      ...
      regex: this.regexp,
      safeValue: (C) => this.escapeValue ? Sr(this.escape(C)) : Sr(C)
    }].forEach((C) => {
      for (o = 0; a = C.regex.exec(e); ) {
        ...
        else !ce(s) && !this.useRawValueToEscape && (s = Ui(s));
        const I = C.safeValue(s);
        if (e = e.replace(a[0], I), h ? (C.regex.lastIndex += s.length, C.regex.lastIndex -= a[0].length) : C.regex.lastIndex = 0, o++, o >= this.maxReplaces)
          break;
      }
    }), e;
  }
  nest(e, n, r = {}) {
    ...
    const o = (c, u) => {
      ...
      try {
        ...
        return this.logger.warn(`failed parsing options string in nesting for key ${c}`, k), `${c}${l}${g}`;
      }
    ...
    for (; i = this.nestingRegexp.exec(e); ) {
      ...
      if (u !== -1 && (c = i[1].slice(u).split(this.formatSeparator).map((l) => l.trim()).filter(Boolean), i[1] = i[1].slice(0, u)), a = n(o.call(this, i[1].trim(), s), s), a && i[0] === e && !ce(a)) return a;
      ce(a) || (a = Ui(a)), a || (this.logger.warn(`missed to resolve ${i[1]} for nesting ${e}`), a = ""), c.length && (a = c.reduce((l, h) => this.format(l, h, r.lng, {
        ...r,
...
const Dl = (t) => {
  ...
  return (n, r, i) => {
    ...
    let o = e[s];
    return o || (o = t(Rn(r), i), e[s] = o), o(n);
  };
}, Fl = (t) => (e, n, r) => t(Rn(n), r)(e);
class Hl {
  constructor(e = {}) {
    this.logger = _t.create("formatter"), this.options = e, this.init(e);
  }
  init(e, n = {
    interpolation: {}
  }) {
    this.formatSeparator = n.interpolation.formatSeparator || ",";
    const r = n.cacheInBuiltFormats ? Xi : Fl;
    this.formats = {
      number: r((i, a) => {
...
class Ul extends ir {
  ...
  queueLoad(e, n, r, i) {
    ...
    return e.forEach((u) => {
      ...
      n.forEach((h) => {
        const g = `${u}|${h}`;
        !r.reload && this.store.hasResourceBundle(u, h) ? this.state[g] = 2 : this.state[g] < 0 || (this.state[g] === 1 ? s[g] === void 0 && (s[g] = !0) : (this.state[g] = 1, l = !1, s[g] === void 0 && (s[g] = !0), a[g] === void 0 && (a[g] = !0), c[h] === void 0 && (c[h] = !0)));
      }), l || (o[u] = !0);
  ...
  loaded(e, n, r) {
    ...
    this.queue.forEach((c) => {
      bl(c.loaded, [a], s), zl(c, e), n && c.errors.push(n), c.pendingCount === 0 && !c.done && (Object.keys(c.loaded).forEach((u) => {
        o[u] || (o[u] = {});
        const l = c.loaded[u];
    ...
    if (!(r == null || r === "")) {
      if ((g = this.backend) != null && g.create) {
        ...
        if (C.length < 6)
          try {
            let k;
            C.length === 5 ? k = C(e, n, r, i, d) : k = C(e, n, r, i), k && typeof k.then == "function" ? k.then((I) => o(null, I)).catch(o) : o(null, k);
          } catch (k) {
            o(k);
          }
        else
...
class Tn extends ir {
  constructor(e = {}, n) {
    if (super(), this.options = Ji(e), this.services = {}, this.logger = _t, this.modules = {
      external: []
    }, Bl(this), n && !this.isInitialized && !e.isClone) {
  ...
  init(e = {}, n) {
    ...
    if (!this.options.isClone) {
      this.modules.logger ? _t.init(i(this.modules.logger), this.options) : _t.init(null, this.options);
      let u;
      ...
      h.logger = _t, h.resourceStore = this.store, h.languageUtils = l, h.pluralResolver = new Pl(l, {
      ...
      }, h.backendConnector = new Ul(i(this.modules.backend), h.resourceStore, h, this.options), h.backendConnector.on("*", (d, ...C) => {
        this.emit(d, ...C);
  ...
  use(e) {
    ...
    return e.type === "backend" && (this.modules.backend = e), (e.type === "logger" || e.log && e.warn && e.error) && (this.modules.logger = e), e.type === "languageDetector" && (this.modules.languageDetector = e), e.type === "i18nFormat" && (this.modules.i18nFormat = e), e.type === "postProcessor" && os.addPostProcessor(e), e.type === "formatter" && (this.modules.formatter = e), e.type === "3rdParty" && this.modules.external.push(e), this;
  }
  ...
  changeLanguage(e, n) {
    ...
    const i = (o) => {
      ...
      const c = ce(o) ? o : o && o[0], u = this.store.hasLanguageSomeTranslations(c) ? c : this.services.languageUtils.getBestMatchFromCodes(ce(o) ? [o] : o);
      u && (this.language || i(u), this.translator.language || this.translator.changeLanguage(u), (h = (l = this.services.languageDetector) == null ? void 0 : l.cacheUserLanguage) == null || h.call(l, u)), this.loadResources(u, (g) => {
        a(g, u);
      });
    };
  ...
  getFixedT(e, n, r) {
    const i = (a, s, ...o) => {
      ...
      let l;
      return c.keyPrefix && Array.isArray(a) ? l = a.map((h) => (typeof h == "function" && (h = Br(h, {
        ...this.options,
        ...s
      })), `${c.keyPrefix}${u}${h}`)) : (typeof a == "function" && (a = Br(a, {
        ...this.options,
        ...s
  ...
  static createInstance(e = {}, n) {
    const r = new Tn(e, n);
    return r.createInstance = Tn.createInstance, r;
  }
...
const $l = (t, e, n, r) => {
  ...
  Yt(i[0]) && (i[0] = `react-i18next:: ${i[0]}`), (c = (o = t == null ? void 0 : t.services) == null ? void 0 : o.logger) != null && c.warn ? t.services.logger.warn(...i) : console != null && console.warn && console.warn(...i);
}, Qi = {}, cs = (t, e, n, r) => {
  Yt(n) && Qi[n] || (Yt(n) && (Qi[n] = /* @__PURE__ */ new Date()), $l(t, e, n, r));
}, us = (t, e) => () => {
  if (t.isInitialized)
    e();
  else {
  ...
  t.loadNamespaces(e, us(t, n));
}, ea = (t, e, n, r) => {
  if (Yt(n) && (n = [n]), t.options.preload && t.options.preload.indexOf(e) > -1) return $r(t, n, r);
  n.forEach((i) => {
    t.options.ns.indexOf(i) < 0 && t.options.ns.push(i);
  }), t.loadLanguages(e, us(t, r));
}, jl = (t, e, n = {}) => !e.languages || !e.languages.length ? (cs(e, "NO_LANGUAGES", "i18n.languages were undefined or empty", {
...
}), Yt = (t) => typeof t == "string", Vl = (t) => typeof t == "object" && t !== null, Gl = /&(?:amp|#38|lt|#60|gt|#62|apos|#39|quot|#34|nbsp|#160|copy|#169|reg|#174|hellip|#8230|#x2F|#47);/g, Wl = {
  "&amp;": "&",
  "&#38;": "&",
  "&lt;": "<",
  "&#60;": "<",
  ...
  "&#x2F;": "/",
  "&#47;": "/"
}, ql = (t) => Wl[t], Kl = (t) => t.replace(Gl, ql);
let jr = {
...
var rc = typeof Object.is == "function" ? Object.is : nc, ic = cn.useState, ac = cn.useEffect, sc = cn.useLayoutEffect, oc = cn.useDebugValue;
function lc(t, e) {
  var n = e(), r = ic({ inst: { value: n, getSnapshot: e } }), i = r[0].inst, a = r[1];
  return sc(
    function() {
...
var uc = typeof window > "u" || typeof window.document > "u" || typeof window.document.createElement > "u" ? cc : lc;
ps.useSyncExternalStore = cn.useSyncExternalStore !== void 0 ? cn.useSyncExternalStore : uc;
ds.exports = ps;
var hc = ds.exports;
const dc = (t, e) => Yt(e) ? e : Vl(e) && Yt(e.defaultValue) ? e.defaultValue : Array.isArray(t) ? t[t.length - 1] : t, pc = {
  t: dc,
  ready: !1
}, fc = () => () => {
}, gc = (t, e = {}) => {
  ...
  a && !a.reportNamespaces && (a.reportNamespaces = new tc()), a || cs(a, "NO_I18NEXT_INSTANCE", "useTranslation: You will need to pass in an i18next instance by using initReactI18next");
  const s = ze(() => {
    var D;
    return {
      ...Yl(),
      ...(D = a == null ? void 0 : a.options) == null ? void 0 : D.react,
      ...e
    };
  }, [a, e]), {
    useSuspense: o,
    keyPrefix: c
  } = s, u = i || ((G = a == null ? void 0 : a.options) == null ? void 0 : G.defaultNS), l = Yt(u) ? [u] : u || ["translation"], h = ze(() => l, l);
  (se = (Z = a == null ? void 0 : a.reportNamespaces) == null ? void 0 : Z.addUsedNamespaces) == null || se.call(Z, h);
  const g = de(0), d = ae((D) => {
    if (!a) return fc;
    const {
      bindI18n: M,
    ...
    return M && a.on(M, z), N && a.store.on(N, z), () => {
      M && M.split(" ").forEach((q) => a.off(q, z)), N && N.split(" ").forEach((q) => a.store.off(q, z));
    };
  }, [a, s]), C = de(), k = ae(() => {
    if (!a)
      return pc;
    const D = !!(a.isInitialized || a.initializedStoreOnce) && h.every((ge) => jl(ge, a, s)), M = e.lng || a.language, N = g.current, z = C.current;
    if (z && z.ready === D && z.lng === M && z.keyPrefix === c && z.revision === N)
      return z;
    const te = {
      t: a.getFixedT(M, s.nsMode === "fallback" ? h : h[0], c),
      ready: D,
      lng: M,
      keyPrefix: c,
      revision: N
    };
    return C.current = te, te;
  }, [a, h, c, s, e.lng]), [I, T] = we(0), {
    t: b,
    ready: v
  } = hc.useSyncExternalStore(d, k, k);
  ke(() => {
    if (a && !v && !o) {
      const D = () => T((M) => M + 1);
      e.lng ? ea(a, e.lng, h, D) : $r(a, h, D);
    }
  }, [a, e.lng, h, v, o, I]);
  const O = a || {}, A = de(null), E = de(), U = (D) => {
    const M = Object.getOwnPropertyDescriptors(D);
    M.__original && delete M.__original;
    const N = Object.create(Object.getPrototypeOf(D), M);
    ...
    const D = O, M = D == null ? void 0 : D.language;
    let N = D;
    D && (A.current && A.current.__original === D ? E.current !== M ? (N = U(D), A.current = N, E.current = M) : N = A.current : (N = U(D), A.current = N, E.current = M));
    const z = [b, N, v];
    return z.t = b, z.i18n = N, z.ready = v, z;
  }, [b, O, v, O.resolvedLanguage, O.language, O.languages]);
  if (a && o && !v)
    throw new Promise((D) => {
      const M = () => D();
      e.lng ? ea(a, e.lng, h, M) : $r(a, h, M);
    });
  return B;
};
async function fs({
  ...
  if (!s.ok) {
    if (s.status === 404 && r !== "en")
      return console.warn(`Translations not found for locale '${r}', falling back to 'en'`), fs({
        chatServerUrl: t,
        chatServerKey: e,
        mpAuthToken: n,
        locale: "en"
...
function Cc({
  ...
  const [s] = we(() => mc()), [o, c] = we(!0), [u, l] = we(!1), [h, g] = we(null), d = de(!1), C = ae(
    async (T, b) => {
  ...
  ke(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        if (!T) return;
        await C(v, e), d.current = !0, l(!0);
      } catch (v) {
        if (!T) return;
        console.error("Failed to load translations:", v), g(
          v instanceof Error ? v : new Error("Failed to load translations")
        ), await C(yc, "en"), d.current = !0, l(!0);
      } finally {
        T && c(!1);
      }
    })(), () => {
  ...
  return o && a ? /* @__PURE__ */ p(Ht, { children: a }) : /* @__PURE__ */ p(gs.Provider, { value: I, children: t });
}
function jt() {
  const t = ii(gs);
  if (!t)
...
function wc() {
  const { t } = jt(), e = ae(
    (a, s) => s === !1 ? je.isErrorMessage(a) ? Ke.ERROR : Ke.COMPLETED : je.isCompletedMessage(a) ? Ke.COMPLETED : je.isErrorMessage(a) ? Ke.ERROR : Ke.PROCESSING,
    []
  ), n = ae(
    (a) => je.extractDuration(a, t),
    [t]
  ), r = ae(
    (a) => je.cleanReasoningContent(a),
    []
  ), i = ae(
    (a, s) => {
      switch (je.getMessageType(
        a,
        s
      )) {
        case ie.MESSAGE_TYPES.ERROR:
          return t("chat.reasoning.error");
        case ie.MESSAGE_TYPES.COMPLETED:
          return t("chat.reasoning.completed");
        case ie.MESSAGE_TYPES.THOUGHT:
          return t("chat.reasoning.thought");
        case ie.MESSAGE_TYPES.THINKING:
        default:
...
function Sc() {
  const { t } = jt(), e = ae(
    (r, i) => i === !1 ? r.includes(ie.ERROR_MARKER) ? t("chat.tools.failed") : t("chat.tools.completed") : r.includes(ie.COMPLETED_MARKER) || r.includes("✅") ? t("chat.tools.completed") : r.includes(ie.ERROR_MARKER) ? t("chat.tools.failed") : (r.includes(ie.HANDLING_MARKER), t("chat.tools.executing")),
    [t]
  ), n = ae(
    (r, i) => i === !1 ? r.includes(ie.ERROR_MARKER) ? Ke.ERROR : Ke.COMPLETED : r.includes(ie.COMPLETED_MARKER) || r.includes("✅") ? Ke.COMPLETED : r.includes(ie.ERROR_MARKER) ? Ke.ERROR : Ke.PROCESSING,
    []
  );
  return {
    getToolingTitle: e,
...
function xc({
  ...
  const g = de(/* @__PURE__ */ new Map()), d = de(/* @__PURE__ */ new Map()), C = ae(() => {
    ...
    (O) => {
      if (window.responseTimeoutId && (clearTimeout(window.responseTimeoutId), window.responseTimeoutId = null, c.current || t((A) => {
        var U;
        const E = (U = A.map((B, G) => ({ msg: B, index: G })).filter(({ msg: B }) => B.role === "user").pop()) == null ? void 0 : U.index;
        return E !== void 0 ? A.map(
          (B, G) => G === E && (B.hasError || B.isRetrying) ? { ...B, hasError: !1, errorMessage: void 0, isRetrying: !1 } : B
        ) : A;
      })), c.current)
        u.current += O, s(u.current), n(
          c.current,
          u.current,
          !0
        );
    ...
    (O, A, E) => {
      const { callId: U } = E || {};
      if (o(O), !U) return;
      const B = je.isThinkingMessage(A) && !ie.PATTERNS.DURATION.test(A), G = je.isThinkingMessage(A) && ie.PATTERNS.DURATION.test(A), Z = je.isHandlingMessage(A), se = je.isCompletedMessage(A), D = je.isErrorMessage(A);
      if (B || G) {
        const N = g.current.get(U);
        if (B && !N) {
          C();
          const z = r();
          g.current.set(U, z);
          ...
          t((te) => [...te, q]);
        } else G && N ? (n(N, A, !1), g.current.delete(U)) : N && B && n(N, A, !0);
      }
      const M = d.current.get(U);
      if (Z && !M) {
        C();
        const N = A.match(
          ie.PATTERNS.HANDLING_TOOL
        ), z = N ? N[1] : "Unknown Tool", q = r();
        d.current.set(U, q);
        const te = {
          id: q,
          role: "tooling",
        ...
        const N = A.match(
          ie.PATTERNS.COMPLETED_OR_ERROR_TOOL
        ), z = N ? N[1] : "Unknown Tool";
        t(
          (q) => q.map(
            (te) => te.id === M ? {
              ...te,
              ...
              toolData: {
                ...te.toolData,
                toolName: z,
                status: D ? Ke.ERROR : Ke.COMPLETED,
                callId: U ?? ""
              }
            } : te
          )
        ), d.current.delete(U);
      } else M && O && !se && !D && n(M, A, !0);
    },
    [
      o,
      C,
    ...
    (O) => {
      a(!1), i(!1), C(), e("system", `❌ Chat error: ${O}`);
    },
    [
      a,
      i,
...
function kc() {
  const t = sl(), e = kl(), n = wc(), r = Sc(), i = xc({
    // From useMessages
...
function Sf({ initialMode: t = "sidebar" }) {
  const e = le();
  return ke(() => {
...
function Ec({
  ...
  return ke(() => {
    ...
    const C = !h.current && n, k = o.current !== t;
    if (C) {
...
function bc() {
  const [t, e] = we(navigator.onLine), [n, r] = we(!1);
  return ke(() => {
    const i = () => {
      e(!0), n && r(!1);
    }, a = () => {
      ...
      c.upload.addEventListener("progress", (u) => {
        ...
        if (c.status >= 200 && c.status < 300)
          try {
            ...
            let l;
            u.data && Array.isArray(u.data) ? l = u.data.map((h, g) => this.processUploadResult(e[g], h)) : Array.isArray(u) ? l = u.map((h, g) => this.processUploadResult(e[g], h)) : l = [this.processUploadResult(e[0], u)], a.forEach((h) => {
...
const Rc = {
   ...
   */
  getContainerClasses: (t, e, n, r, i) => ys.buildClasses(
    "chat-wrapper",
    `chat-wrapper--${t}`,
    e && `chat-wrapper--${e}`,
    n && `chat-wrapper--${n}`,
...
class na extends ai {
  ...
  render() {
    ...
    return n && r ? a ? a(r, this.handleRetry) : /* @__PURE__ */ p("div", { className: "chat-wrapper__error-boundary", children: /* @__PURE__ */ P("div", { className: "chat-wrapper__error-content", children: [
      ...
      /* @__PURE__ */ p("p", { className: "chat-wrapper__error-message", children: Et.error.getUserFriendlyErrorMessage(r) }),
...
class Mc extends ai {
  ...
  render() {
    ...
    return n && r && (r.message.includes("WebSocket") || r.message.includes("connection") || Et.error.isNetworkError(r)) ? /* @__PURE__ */ p("div", { className: "chat-wrapper__websocket-error", children: /* @__PURE__ */ P("div", { className: "chat-wrapper__error-content", children: [
...
const Ac = ({
  ...
  return /* @__PURE__ */ P(
    "button",
    {
      ...
      title: a,
      children: [
        /* @__PURE__ */ p(Ac, { className: "chat-wrapper__bubble-icon", size: 24 }),
  ...
  const o = () => e === "modal" && r && i ? /* @__PURE__ */ p(
    "button",
    {
      ...
      onClick: i,
      title: "Close chat",
      children: /* @__PURE__ */ p(Lc, { size: 20 })
    }
  ) : null, c = () => {
    if ((e === "sidebar" || e === "fullscreen") && !n && a) {
      ...
      return /* @__PURE__ */ p(
        "button",
        {
          ...
          children: /* @__PURE__ */ p(Oc, { size: 20, isFullscreen: l })
        }
      );
    }
    return null;
    ...
    "button",
    {
      ...
      onClick: s,
      title: "Collapse chat",
      children: /* @__PURE__ */ p(Pc, { size: 20 })
    }
  ) : null;
...
function Vr(t) {
  ws(!1, !1, !0, "ok", "Unreachable", t);
}
function ws(t, e, n, r, i, a) {
  if (!t)
    throw a instanceof Error ? a : new zc(
      a || i,
      e,
      n,
      r,
...
const Bc = /^[$_\p{ID_Start}][$_\u{200C}\u{200D}\p{ID_Continue}]*$/u, $c = /^[$_\p{ID_Start}][-$_\u{200C}\u{200D}\p{ID_Continue}]*$/u, jc = {};
function ra(t, e) {
  return (jc.jsx ? $c : Bc).test(t);
}
const Vc = /[ \t\n\f\r]/g;
function Gc(t) {
...
let Wc = 0;
const pe = Xt(), Fe = Xt(), Wr = Xt(), F = Xt(), _e = Xt(), sn = Xt(), ht = Xt();
function Xt() {
  return 2 ** ++Wc;
}
...
}, Yc = /[A-Z]/g, sa = /-[a-z]/g, Xc = /^data[-\w.:]+$/i;
function Jc(t, e) {
  const n = Gr(e);
  let r = e, i = it;
  if (n in t.normal)
    return t.property[t.normal[n]];
  if (n.length > 4 && n.slice(0, 4) === "data" && Xc.test(e)) {
    if (e.charAt(4) === "-") {
      const a = e.slice(5).replace(sa, eu);
      r = "data" + a.charAt(0).toUpperCase() + a.slice(1);
    } else {
      const a = e.slice(4);
      if (!sa.test(a)) {
        let s = a.replace(Yc, Qc);
...
const tu = Ss([xs, qc, Es, bs, _s], "html"), ci = Ss([xs, Kc, Es, bs, _s], "svg");
function nu(t) {
  return t.join(" ").trim();
}
var ui = {}, oa = /\/\*[^*]*\*+([^/*][^*]*\*+)*\//g, ru = /\n/g, iu = /^\s*/, au = /^(\*?[-#/*\\\w]+(\[[0-9a-z_-]+\])?)\s*/, su = /^:\s*/, ou = /^((?:'(?:\\'|.)*?'|"(?:\\"|.)*?"|\([^)]*?\)|[^};])+)/, lu = /^[;\s]*/, cu = /^\s+|\s+$/g, uu = `
`, la = "/", ca = "*", Zt = "", hu = "comment", du = "declaration", pu = function(t, e) {
  if (typeof t != "string")
  ...
  function h() {
    ...
    if (!(la != t.charAt(0) || ca != t.charAt(1))) {
      for (var k = 2; Zt != t.charAt(k) && (ca != t.charAt(k) || la != t.charAt(k + 1)); )
        ++k;
      if (k += 2, Zt === t.charAt(k - 1))
  ...
  function g() {
    var C = a(), k = c(au);
    if (k) {
      if (h(), !c(su)) return o("property missing ':'");
      var I = c(ou), T = C({
        type: du,
        property: ua(k[0].replace(oa, Zt)),
        value: I ? ua(I[0].replace(oa, Zt)) : Zt
      });
      return c(lu), T;
    }
  }
...
function ua(t) {
  return t ? t.replace(cu, Zt) : Zt;
}
var fu = tr && tr.__importDefault || function(t) {
...
function mu(t, e) {
  ...
  if (!t || typeof t != "string")
    return n;
  var r = (0, gu.default)(t), i = typeof e == "function";
...
ar.camelCase = void 0;
var yu = /^--[a-zA-Z0-9_-]+$/, Cu = /-([a-z])/g, wu = /^[^-]+$/, Su = /^-(webkit|moz|ms|o|khtml)-/, xu = /^-(ms)-/, ku = function(t) {
  return !t || wu.test(t) || yu.test(t);
}, Tu = function(t, e) {
  return e.toUpperCase();
}, ha = function(t, e) {
  return "".concat(e, "-");
}, Eu = function(t, e) {
  return e === void 0 && (e = {}), ku(t) ? t : (t = t.toLowerCase(), e.reactCompat ? t = t.replace(xu, ha) : t = t.replace(Su, ha), t.replace(Cu, Tu));
};
ar.camelCase = Eu;
var bu = tr && tr.__importDefault || function(t) {
  return t && t.__esModule ? t : { default: t };
}, _u = bu(ui), vu = ar;
function Kr(t, e) {
  var n = {};
  return !t || typeof t != "string" || (0, _u.default)(t, function(r, i) {
    r && i && (n[(0, vu.camelCase)(r, e)] = i);
  }), n;
}
Kr.default = Kr;
var Ru = Kr;
const Iu = /* @__PURE__ */ oi(Ru), vs = Rs("end"), hi = Rs("start");
function Rs(t) {
  return e;
  function e(n) {
...
function En(t) {
  return !t || typeof t != "object" ? "" : "position" in t || "type" in t ? da(t.position) : "start" in t || "end" in t ? da(t) : "line" in t || "column" in t ? Zr(t) : "";
}
function Zr(t) {
...
const di = {}.hasOwnProperty, Nu = /* @__PURE__ */ new Map(), Au = /[A-Z]/g, Lu = /* @__PURE__ */ new Set(["table", "tbody", "thead", "tfoot", "tr"]), Ou = /* @__PURE__ */ new Set(["td", "th"]), Is = "https://github.com/syntax-tree/hast-util-to-jsx-runtime";
function Pu(t, e) {
...
function Du(t, e, n) {
  const r = t.schema;
  let i = r;
  e.tagName.toLowerCase() === "svg" && r.space === "html" && (i = ci, t.schema = i), t.ancestors.push(e);
  const a = As(t, e.tagName, !1), s = Vu(t, e);
  let o = fi(t, e);
  return Lu.has(e.tagName) && (o = o.filter(function(c) {
    return typeof c == "string" ? !Gc(c) : !0;
  })), Ns(t, s, a, e), pi(s, o), t.ancestors.pop(), t.schema = r, t.create(e, a, s, n);
}
function Fu(t, e) {
...
function zu(t, e, n) {
  const r = t.schema;
  let i = r;
  e.name === "svg" && r.space === "html" && (i = ci, t.schema = i), t.ancestors.push(e);
  const a = e.name === null ? t.Fragment : As(t, e.name, !0), s = Gu(t, e), o = fi(t, e);
  return Ns(t, s, a, e), pi(s, o), t.ancestors.pop(), t.schema = r, t.create(e, a, s, n);
}
function Uu(t, e, n) {
  const r = {};
  return pi(r, fi(t, e)), t.create(e, t.Fragment, r, n);
}
function Bu(t, e) {
  return e.value;
}
...
function ju(t, e) {
  return n;
  function n(r, i, a, s) {
    const o = Array.isArray(a.children), c = hi(r);
    return e(
      i,
      a,
      s,
...
function Vu(t, e) {
  ...
  for (i in e.properties)
    if (i !== "children" && di.call(e.properties, i)) {
      const a = Wu(t, i, e.properties[i]);
      if (a) {
        const [s, o] = a;
        t.tableCellAlignToStyle && s === "align" && typeof o == "string" && Ou.has(e.tagName) ? r = o : n[s] = o;
      }
    }
  if (r) {
    const a = (
...
function fi(t, e) {
  const n = [];
  let r = -1;
  const i = t.passKeys ? /* @__PURE__ */ new Map() : Nu;
...
function Wu(t, e, n) {
  const r = Jc(t.schema, e);
  if (!(n == null || typeof n == "number" && Number.isNaN(n))) {
    if (Array.isArray(n) && (n = r.commaSeparated ? Uc(n) : nu(n)), r.property === "style") {
      let i = typeof n == "object" ? n : qu(t, String(n));
      return t.stylePropertyNameCase === "css" && (i = Ku(i)), ["style", i];
    }
    return [
      t.elementAttributeNameCase === "react" && r.space ? Zc[r.property] || r.property : r.attribute,
      n
    ];
  }
}
function qu(t, e) {
  try {
    ...
    throw i.file = t.filePath || void 0, i.url = Is + "#cannot-parse-style-attribute", i;
  }
}
function As(t, e, n) {
  let r;
  ...
  if (r.type === "Literal") {
    ...
    return di.call(t.components, i) ? t.components[i] : i;
  }
  if (t.evaluater)
...
function In(t, e) {
  ...
  throw n.file = t.filePath || void 0, n.url = Is + "#cannot-handle-mdx-estrees-without-createevaluater", n;
}
function Ku(t) {
  const e = {};
  let n;
  for (n in t)
    di.call(t, n) && (e[Zu(n)] = t[n]);
  return e;
}
function Zu(t) {
  let e = t.replace(Au, Yu);
...
function Ju(t, e) {
  const n = Xu, r = typeof n.includeImageAlt == "boolean" ? n.includeImageAlt : !0, i = typeof n.includeHtml == "boolean" ? n.includeHtml : !0;
  return Ls(t, r, i);
}
function Ls(t, e, n) {
  if (Qu(t)) {
...
const bt = Vt(/[A-Za-z]/), pt = Vt(/[\dA-Za-z]/), rh = Vt(/[#-'*+\--9=?A-Z^-~]/);
function Yr(t) {
  return (
...
const Xr = Vt(/\d/), ih = Vt(/[\dA-Fa-f]/), ah = Vt(/[!-/:-@[-`{-~]/);
function K(t) {
...
const sh = Vt(new RegExp("\\p{P}|\\p{S}", "u")), oh = Vt(/\s/);
function Vt(t) {
  return e;
  function e(n) {
...
function hn(t) {
  ...
  for (; ++n < t.length; ) {
    ...
    let s = "";
    if (a === m.percentSign && pt(t.charCodeAt(n + 1)) && pt(t.charCodeAt(n + 2)))
      i = 2;
    else if (a < 128)
      /[!#$&-;=?-Z_a-z~]/.test(String.fromCharCode(a)) || (s = String.fromCharCode(a));
    else if (a > 55295 && a < 57344) {
      const o = t.charCodeAt(n + 1);
      a < 56320 && o > 56319 && o < 57344 ? (s = String.fromCharCode(a, o), i = 1) : s = Tt.replacementCharacter;
    } else
...
function ve(t, e, n, r) {
  ...
  return s;
  function s(c) {
    return Ce(c) ? (t.enter(n), o(c)) : e(c);
  }
  function o(c) {
    return Ce(c) && a++ < i ? (t.consume(c), o) : (t.exit(n), e(c));
  }
}
const lh = { tokenize: ch };
function ch(t) {
  const e = t.attempt(
  ...
  function r(o) {
    ...
    return t.enter(f.lineEnding), t.consume(o), t.exit(f.lineEnding), ve(t, e, f.linePrefix);
  }
  function i(o) {
    return x(
      o !== m.eof && !K(o),
  ...
  function s(o) {
    ...
    return K(o) ? (t.consume(o), t.exit(f.chunkText), a) : (t.consume(o), s);
  }
}
const uh = { tokenize: hh }, ya = { tokenize: dh };
function hh(t) {
  const e = this, n = [];
  let r = 0, i, a, s;
  return o;
  ...
  function c(v) {
    if (x(
      ...
      let A = O, E;
      for (; A--; )
        if (e.events[A][0] === "exit" && e.events[A][1].type === f.chunkFlow) {
          E = e.events[A][1].end;
          break;
        }
      x(E, "could not find previous flow chunk"), T(r);
      let U = O;
  ...
  function h(v) {
    return e.parser.lazy[e.now().line] = r !== n.length, s = e.now().offset, C(v);
  }
  function g(v) {
  ...
  function d(v) {
    ...
    ), r++, n.push([e.currentConstruct, e.containerState]), g(v);
  }
  function C(v) {
    if (v === m.eof) {
      i && b(), T(0), t.consume(v);
  ...
  function k(v) {
    if (v === m.eof) {
      I(t.exit(f.chunkFlow), !0), T(0), t.consume(v);
      return;
    }
    return K(v) ? (t.consume(v), I(t.exit(f.chunkFlow)), r = 0, e.interrupt = void 0, o) : (t.consume(v), k);
  }
  function I(v, O) {
    x(i, "expected `childFlow` to be defined when continuing");
    const A = e.sliceStream(v);
    if (O && A.push(null), v.previous = a, a && (a.next = v), a = v, i.defineSkip(v.start), i.write(A), e.parser.lazy[v.start.line]) {
      ...
      let B = U, G, Z;
      for (; B--; )
        if (e.events[B][0] === "exit" && e.events[B][1].type === f.chunkFlow) {
          if (G) {
            Z = e.events[B][1].end;
            break;
          }
      ...
      for (x(Z, "could not find previous flow chunk"), T(r), E = U; E < e.events.length; )
    ...
    if (t[n][0] === "enter" && t[n][1].type === "attentionSequence" && t[n][1]._close) {
      for (r = n; r--; )
        ...
        e.sliceSerialize(t[r][1]).charCodeAt(0) === e.sliceSerialize(t[n][1]).charCodeAt(0)) {
          ...
          const h = { ...t[r][1].end }, g = { ...t[n][1].start };
          wa(h, -c), wa(g, c), s = {
            type: c > 1 ? f.strongSequence : f.emphasisSequence,
            start: h,
            end: { ...t[r][1].end }
          }, o = {
            type: c > 1 ? f.strongSequence : f.emphasisSequence,
            ...
            type: c > 1 ? f.strongText : f.emphasisText,
            start: { ...t[r][1].end },
            end: { ...t[n][1].start }
          }, i = {
            type: c > 1 ? f.strong : f.emphasis,
            start: { ...s.start },
            end: { ...o.end }
          }, t[r][1].end = { ...s.start }, t[n][1].start = { ...o.end }, u = [], t[r][1].end.offset - t[r][1].start.offset && (u = gt(u, [
            ["enter", t[r][1], e],
            ["exit", t[r][1], e]
          ])), u = gt(u, [
            ["enter", i, e],
            ...
            ["exit", o, e],
            ["exit", i, e]
          ]), t[n][1].end.offset - t[n][1].start.offset ? (l = 2, u = gt(u, [
            ["enter", t[n][1], e],
            ["exit", t[n][1], e]
          ])) : l = 0, vt(t, r - 1, n - r + 3, u), n = r + u.length - l - 2;
          break;
        }
    }
  for (n = -1; ++n < t.length; )
...
function fh(t, e) {
  ...
  function o(c) {
    ...
    const h = !l || l === j.characterGroupPunctuation && i || n.includes(c), g = !i || i === j.characterGroupPunctuation && l || n.includes(r);
    return u._open = !!(a === m.asterisk ? h : h && (i || !g)), u._close = !!(a === m.asterisk ? g : g && (l || !h)), e(c);
  }
}
function wa(t, e) {
  t.column += e, t.offset += e, t._bufferIndex += e;
}
const gh = { name: "autolink", tokenize: mh };
function mh(t, e, n) {
  let r = 0;
  return i;
  function i(d) {
    return x(d === m.lessThan, "expected `<`"), t.enter(f.autolink), t.enter(f.autolinkMarker), t.consume(d), t.exit(f.autolinkMarker), t.enter(f.autolinkProtocol), a;
  }
  function a(d) {
    return bt(d) ? (t.consume(d), s) : d === m.atSign ? n(d) : u(d);
  }
  function s(d) {
    return d === m.plusSign || d === m.dash || d === m.dot || pt(d) ? (r = 1, o(d)) : u(d);
  }
  function o(d) {
    return d === m.colon ? (t.consume(d), r = 0, c) : (d === m.plusSign || d === m.dash || d === m.dot || pt(d)) && r++ < j.autolinkSchemeSizeMax ? (t.consume(d), o) : (r = 0, u(d));
  }
  function c(d) {
    return d === m.greaterThan ? (t.exit(f.autolinkProtocol), t.enter(f.autolinkMarker), t.consume(d), t.exit(f.autolinkMarker), t.exit(f.autolink), e) : d === m.eof || d === m.space || d === m.lessThan || Yr(d) ? n(d) : (t.consume(d), c);
  }
  function u(d) {
    return d === m.atSign ? (t.consume(d), l) : rh(d) ? (t.consume(d), u) : n(d);
  }
  function l(d) {
    return pt(d) ? h(d) : n(d);
  }
  function h(d) {
    return d === m.dot ? (t.consume(d), r = 0, l) : d === m.greaterThan ? (t.exit(f.autolinkProtocol).type = f.autolinkEmail, t.enter(f.autolinkMarker), t.consume(d), t.exit(f.autolinkMarker), t.exit(f.autolink), e) : g(d);
  }
  function g(d) {
    if ((d === m.dash || pt(d)) && r++ < j.autolinkDomainSizeMax) {
      const C = d === m.dash ? g : h;
      return t.consume(d), C;
    }
    return n(d);
  }
}
const sr = { partial: !0, tokenize: yh };
function yh(t, e, n) {
  return r;
  function r(a) {
    return Ce(a) ? ve(t, i, f.linePrefix)(a) : i(a);
  }
  function i(a) {
    return a === m.eof || K(a) ? e(a) : n(a);
  }
}
const Ps = {
  continuation: { tokenize: wh },
  exit: Sh,
  name: "blockQuote",
  tokenize: Ch
};
function Ch(t, e, n) {
  ...
  function a(s) {
    return Ce(s) ? (t.enter(f.blockQuotePrefixWhitespace), t.consume(s), t.exit(f.blockQuotePrefixWhitespace), t.exit(f.blockQuotePrefix), e) : (t.exit(f.blockQuotePrefix), e(s));
  }
}
function wh(t, e, n) {
  const r = this;
...
function xh(t, e, n) {
  ...
  function i(a) {
    return ah(a) ? (t.enter(f.characterEscapeValue), t.consume(a), t.exit(f.characterEscapeValue), t.exit(f.characterEscape), e) : n(a);
  }
}
const Fs = {
...
function kh(t, e, n) {
  ...
  return o;
  function o(h) {
    return x(h === m.ampersand, "expected `&`"), t.enter(f.characterReference), t.enter(f.characterReferenceMarker), t.consume(h), t.exit(f.characterReferenceMarker), c;
  }
  function c(h) {
    return h === m.numberSign ? (t.enter(f.characterReferenceMarkerNumeric), t.consume(h), t.exit(f.characterReferenceMarkerNumeric), u) : (t.enter(f.characterReferenceValue), a = j.characterReferenceNamedSizeMax, s = pt, l(h));
  }
  function u(h) {
    return h === m.uppercaseX || h === m.lowercaseX ? (t.enter(f.characterReferenceMarkerHexadecimal), t.consume(h), t.exit(f.characterReferenceMarkerHexadecimal), t.enter(f.characterReferenceValue), a = j.characterReferenceHexadecimalSizeMax, s = ih, l) : (t.enter(f.characterReferenceValue), a = j.characterReferenceDecimalSizeMax, s = Xr, l(h));
  }
  function l(h) {
    if (h === m.semicolon && i) {
      const g = t.exit(f.characterReferenceValue);
      return s === pt && !gi(r.sliceSerialize(g)) ? n(h) : (t.enter(f.characterReferenceMarker), t.consume(h), t.exit(f.characterReferenceMarker), t.exit(f.characterReference), e);
    }
...
function Th(t, e, n) {
  const r = this, i = { partial: !0, tokenize: A };
  let a = 0, s = 0, o;
  return c;
  function c(E) {
    return u(E);
  ...
  function u(E) {
    x(
      E === m.graveAccent || E === m.tilde,
      "expected `` ` `` or `~`"
    );
    const U = r.events[r.events.length - 1];
    return a = U && U[1].type === f.linePrefix ? U[2].sliceSerialize(U[1], !0).length : 0, o = E, t.enter(f.codeFenced), t.enter(f.codeFencedFence), t.enter(f.codeFencedFenceSequence), l(E);
  }
  function l(E) {
    return E === o ? (s++, t.consume(E), l) : s < j.codeFencedSequenceSizeMin ? n(E) : (t.exit(f.codeFencedFenceSequence), Ce(E) ? ve(t, h, f.whitespace)(E) : h(E));
  }
  function h(E) {
    return E === m.eof || K(E) ? (t.exit(f.codeFencedFence), r.interrupt ? e(E) : t.check(Sa, k, O)(E)) : (t.enter(f.codeFencedFenceInfo), t.enter(f.chunkString, { contentType: j.contentTypeString }), g(E));
  }
  function g(E) {
    return E === m.eof || K(E) ? (t.exit(f.chunkString), t.exit(f.codeFencedFenceInfo), h(E)) : Ce(E) ? (t.exit(f.chunkString), t.exit(f.codeFencedFenceInfo), ve(t, d, f.whitespace)(E)) : E === m.graveAccent && E === o ? n(E) : (t.consume(E), g);
  }
  function d(E) {
    return E === m.eof || K(E) ? h(E) : (t.enter(f.codeFencedFenceMeta), t.enter(f.chunkString, { contentType: j.contentTypeString }), C(E));
  }
  function C(E) {
    return E === m.eof || K(E) ? (t.exit(f.chunkString), t.exit(f.codeFencedFenceMeta), h(E)) : E === m.graveAccent && E === o ? n(E) : (t.consume(E), C);
  }
  function k(E) {
    return x(K(E), "expected eol"), t.attempt(i, O, I)(E);
  }
  function I(E) {
    return x(K(E), "expected eol"), t.enter(f.lineEnding), t.consume(E), t.exit(f.lineEnding), T;
  }
  function T(E) {
    return a > 0 && Ce(E) ? ve(
      t,
  ...
  function b(E) {
    return E === m.eof || K(E) ? t.check(Sa, k, O)(E) : (t.enter(f.codeFlowValue), v(E));
  }
  function v(E) {
    return E === m.eof || K(E) ? (t.exit(f.codeFlowValue), b(E)) : (t.consume(E), v);
  }
  function O(E) {
    return t.exit(f.codeFenced), e(E);
  }
  function A(E, U, B) {
    let G = 0;
    return Z;
    function Z(z) {
      return x(K(z), "expected eol"), E.enter(f.lineEnding), E.consume(z), E.exit(f.lineEnding), se;
    }
    function se(z) {
      return x(
        r.parser.constructs.disable.null,
        "expected `disable.null` to be populated"
      ), E.enter(f.codeFencedFence), Ce(z) ? ve(
        E,
        D,
        f.linePrefix,
        r.parser.constructs.disable.null.includes("codeIndented") ? void 0 : j.tabSize
    ...
    function D(z) {
      return z === o ? (E.enter(f.codeFencedFenceSequence), M(z)) : B(z);
    }
    function M(z) {
      return z === o ? (G++, E.consume(z), M) : G >= s ? (E.exit(f.codeFencedFenceSequence), Ce(z) ? ve(E, N, f.whitespace)(z) : N(z)) : B(z);
    }
    function N(z) {
      return z === m.eof || K(z) ? (E.exit(f.codeFencedFence), U(z)) : B(z);
    }
  }
}
function Eh(t, e, n) {
  ...
  return i;
  function i(s) {
    return s === m.eof ? n(s) : (x(K(s), "expected eol"), t.enter(f.lineEnding), t.consume(s), t.exit(f.lineEnding), a);
  }
  function a(s) {
...
const br = {
  name: "codeIndented",
  tokenize: _h
}, bh = { partial: !0, tokenize: vh };
function _h(t, e, n) {
  const r = this;
  return i;
  function i(u) {
    return x(Ce(u)), t.enter(f.codeIndented), ve(
      t,
      a,
      f.linePrefix,
      j.tabSize + 1
  ...
  function a(u) {
    ...
    return l && l[1].type === f.linePrefix && l[2].sliceSerialize(l[1], !0).length >= j.tabSize ? s(u) : n(u);
  }
  function s(u) {
    return u === m.eof ? c(u) : K(u) ? t.attempt(bh, s, c)(u) : (t.enter(f.codeFlowValue), o(u));
  }
  function o(u) {
    return u === m.eof || K(u) ? (t.exit(f.codeFlowValue), s(u)) : (t.consume(u), o);
  }
  function c(u) {
...
function vh(t, e, n) {
  ...
  return i;
  function i(s) {
    return r.parser.lazy[r.now().line] ? n(s) : K(s) ? (t.enter(f.lineEnding), t.consume(s), t.exit(f.lineEnding), i) : ve(
      t,
      a,
      f.linePrefix,
      j.tabSize + 1
  ...
  function a(s) {
    ...
    return o && o[1].type === f.linePrefix && o[2].sliceSerialize(o[1], !0).length >= j.tabSize ? e(s) : K(s) ? i(s) : n(s);
  }
}
const Rh = {
  name: "codeText",
...
function Mh(t, e, n) {
  ...
  return o;
  function o(g) {
    return x(g === m.graveAccent, "expected `` ` ``"), x(Hs.call(r, r.previous), "expected correct previous"), t.enter(f.codeText), t.enter(f.codeTextSequence), c(g);
  }
  function c(g) {
    return g === m.graveAccent ? (t.consume(g), i++, c) : (t.exit(f.codeTextSequence), u(g));
  }
  function u(g) {
    return g === m.eof ? n(g) : g === m.space ? (t.enter("space"), t.consume(g), t.exit("space"), u) : g === m.graveAccent ? (s = t.enter(f.codeTextSequence), a = 0, h(g)) : K(g) ? (t.enter(f.lineEnding), t.consume(g), t.exit(f.lineEnding), u) : (t.enter(f.codeTextData), l(g));
  }
  function l(g) {
    return g === m.eof || g === m.space || g === m.graveAccent || K(g) ? (t.exit(f.codeTextData), u(g)) : (t.consume(g), l);
  }
  function h(g) {
    return g === m.graveAccent ? (t.consume(g), a++, h) : a === i ? (t.exit(f.codeTextSequence), t.exit(f.codeText), e(g)) : (s.type = f.codeTextData, l(g));
  }
}
class Nh {
  /**
  ...
  splice(e, n, r) {
    ...
    return r && Sn(this.left, r), a.reverse();
  }
  /**
...
function zs(t) {
  ...
  for (; ++n < l.length; ) {
    for (; n in e; )
      n = e[n];
    if (r = l.get(n), n && r[1].type === f.chunkFlow && l.get(n - 1)[1].type === f.listItemPrefix && (x(r[1]._tokenizer, "expected `_tokenizer` on subtokens"), c = r[1]._tokenizer.events, a = 0, a < c.length && c[a][1].type === f.lineEndingBlank && (a += 2), a < c.length && c[a][1].type === f.content))
      for (; ++a < c.length && c[a][1].type !== f.content; )
        c[a][1].type === f.chunkText && (c[a][1]._isInFirstContentOfListItem = !0, a++);
    if (r[0] === "enter")
      r[1].contentType && (Object.assign(e, Ah(l, n)), n = e[n], u = !0);
    else if (r[1]._container) {
      for (a = n, i = void 0; a--; )
        if (s = l.get(a), s[1].type === f.lineEnding || s[1].type === f.lineEndingBlank)
          s[0] === "enter" && (i && (l.get(i)[1].type = f.lineEndingBlank), s[1].type = f.lineEnding, i = a);
  ...
  return vt(t, 0, Number.POSITIVE_INFINITY, l.slice(0)), !u;
}
function Ah(t, e) {
  ...
  for (; d; ) {
    ...
    ), x(!h || h.next === d, "expected next to match"), a.push(i), d._tokenizer || (l = r.sliceStream(d), d.next || l.push(m.eof), h && s.defineSkip(d.start), d._isInFirstContentOfListItem && (s._gfmTasklistFirstContentOfListItem = !0), s.write(l), d._isInFirstContentOfListItem && (s._gfmTasklistFirstContentOfListItem = void 0)), h = d, d = d.next;
  }
  for (d = n; ++g < o.length; )
    // Find a void token that includes a break.
    o[g][0] === "exit" && o[g - 1][0] === "enter" && o[g][1].type === o[g - 1][1].type && o[g][1].start.line !== o[g][1].end.line && (x(d, "expected a current token"), k = g + 1, I.push(k), d._tokenizer = void 0, d.previous = void 0, d = d.next);
  for (s.events = [], d ? (d._tokenizer = void 0, d.previous = void 0, x(!d.next, "expected no next token")) : I.pop(), g = I.length; g--; ) {
    const T = o.slice(I[g], I[g + 1]), b = a.pop();
    x(b !== void 0, "expected a start position when splicing"), c.push([b, b + T.length - 1]), t.splice(b, 2, T);
  }
  for (c.reverse(), g = -1; ++g < c.length; )
    u[C + c[g][0]] = C + c[g][1], C += c[g][1] - c[g][0] - 1;
  return u;
}
const Lh = { resolve: Ph, tokenize: Dh }, Oh = { partial: !0, tokenize: Fh };
function Ph(t) {
  return zs(t), t;
}
function Dh(t, e) {
  ...
  function i(o) {
    return o === m.eof ? a(o) : K(o) ? t.check(
      Oh,
      s,
      a
    )(o) : (t.consume(o), i);
  ...
  function s(o) {
    return x(K(o), "expected eol"), t.consume(o), t.exit(f.chunkContent), x(n, "expected previous token"), n.next = t.enter(f.chunkContent, {
      contentType: j.contentTypeContent,
      previous: n
    }), n = n.next, i;
  }
...
function Fh(t, e, n) {
  ...
  return i;
  function i(s) {
    return x(K(s), "expected a line ending"), t.exit(f.chunkContent), t.enter(f.lineEnding), t.consume(s), t.exit(f.lineEnding), ve(t, a, f.linePrefix);
  }
  function a(s) {
    if (s === m.eof || K(s))
      return n(s);
...
function Us(t, e, n, r, i, a, s, o, c) {
  const u = c || Number.POSITIVE_INFINITY;
  let l = 0;
  return h;
  function h(T) {
    return T === m.lessThan ? (t.enter(r), t.enter(i), t.enter(a), t.consume(T), t.exit(a), g) : T === m.eof || T === m.space || T === m.rightParenthesis || Yr(T) ? n(T) : (t.enter(r), t.enter(s), t.enter(o), t.enter(f.chunkString, { contentType: j.contentTypeString }), k(T));
  }
  function g(T) {
    return T === m.greaterThan ? (t.enter(a), t.consume(T), t.exit(a), t.exit(i), t.exit(r), e) : (t.enter(o), t.enter(f.chunkString, { contentType: j.contentTypeString }), d(T));
  }
  function d(T) {
    return T === m.greaterThan ? (t.exit(f.chunkString), t.exit(o), g(T)) : T === m.eof || T === m.lessThan || K(T) ? n(T) : (t.consume(T), T === m.backslash ? C : d);
  }
  function C(T) {
    return T === m.lessThan || T === m.greaterThan || T === m.backslash ? (t.consume(T), d) : d(T);
  }
  function k(T) {
    return !l && (T === m.eof || T === m.rightParenthesis || rt(T)) ? (t.exit(f.chunkString), t.exit(o), t.exit(s), t.exit(r), e(T)) : l < u && T === m.leftParenthesis ? (t.consume(T), l++, k) : T === m.rightParenthesis ? (t.consume(T), l--, k) : T === m.eof || T === m.space || T === m.leftParenthesis || Yr(T) ? n(T) : (t.consume(T), T === m.backslash ? I : k);
  }
  function I(T) {
    return T === m.leftParenthesis || T === m.rightParenthesis || T === m.backslash ? (t.consume(T), k) : k(T);
  }
}
function Bs(t, e, n, r, i, a) {
  ...
  return u;
  function u(d) {
    return x(d === m.leftSquareBracket, "expected `[`"), t.enter(r), t.enter(i), t.consume(d), t.exit(i), t.enter(a), l;
  }
  function l(d) {
    return o > j.linkReferenceSizeMax || d === m.eof || d === m.leftSquareBracket || d === m.rightSquareBracket && !c || // To do: remove in the future once we’ve switched from
    // `micromark-extension-footnote` to `micromark-extension-gfm-footnote`,
    // which doesn’t need this.
    // Hidden footnotes hook.
    /* c8 ignore next 3 */
    d === m.caret && !o && "_hiddenFootnoteSupport" in s.parser.constructs ? n(d) : d === m.rightSquareBracket ? (t.exit(a), t.enter(i), t.consume(d), t.exit(i), t.exit(r), e) : K(d) ? (t.enter(f.lineEnding), t.consume(d), t.exit(f.lineEnding), l) : (t.enter(f.chunkString, { contentType: j.contentTypeString }), h(d));
  }
  function h(d) {
    return d === m.eof || d === m.leftSquareBracket || d === m.rightSquareBracket || K(d) || o++ > j.linkReferenceSizeMax ? (t.exit(f.chunkString), l(d)) : (t.consume(d), c || (c = !Ce(d)), d === m.backslash ? g : h);
  }
  function g(d) {
    return d === m.leftSquareBracket || d === m.backslash || d === m.rightSquareBracket ? (t.consume(d), o++, h) : h(d);
  }
}
function $s(t, e, n, r, i, a) {
  ...
  return o;
  function o(g) {
    return g === m.quotationMark || g === m.apostrophe || g === m.leftParenthesis ? (t.enter(r), t.enter(i), t.consume(g), t.exit(i), s = g === m.leftParenthesis ? m.rightParenthesis : g, c) : n(g);
  }
  function c(g) {
    return g === s ? (t.enter(i), t.consume(g), t.exit(i), t.exit(r), e) : (t.enter(a), u(g));
  }
  function u(g) {
    return g === s ? (t.exit(a), c(s)) : g === m.eof ? n(g) : K(g) ? (t.enter(f.lineEnding), t.consume(g), t.exit(f.lineEnding), ve(t, u, f.linePrefix)) : (t.enter(f.chunkString, { contentType: j.contentTypeString }), l(g));
  }
  function l(g) {
    return g === s || g === m.eof || K(g) ? (t.exit(f.chunkString), u(g)) : (t.consume(g), g === m.backslash ? h : l);
  }
  function h(g) {
    return g === s || g === m.backslash ? (t.consume(g), l) : l(g);
  }
}
function bn(t, e) {
  let n;
  return r;
  function r(i) {
    return K(i) ? (t.enter(f.lineEnding), t.consume(i), t.exit(f.lineEnding), n = !0, r) : Ce(i) ? ve(
      t,
      r,
...
const Hh = { name: "definition", tokenize: Uh }, zh = { partial: !0, tokenize: Bh };
function Uh(t, e, n) {
  const r = this;
  let i;
  return a;
  function a(d) {
    return t.enter(f.definition), s(d);
  }
  function s(d) {
    return x(d === m.leftSquareBracket, "expected `[`"), Bs.call(
      r,
      t,
      o,
  ...
  function o(d) {
    ...
    ), d === m.colon ? (t.enter(f.definitionMarker), t.consume(d), t.exit(f.definitionMarker), c) : n(d);
  }
  function c(d) {
    return rt(d) ? bn(t, u)(d) : u(d);
  }
  function u(d) {
    return Us(
      t,
  ...
  function l(d) {
    return t.attempt(zh, h, h)(d);
  }
  function h(d) {
    return Ce(d) ? ve(t, g, f.whitespace)(d) : g(d);
  }
  function g(d) {
    return d === m.eof || K(d) ? (t.exit(f.definition), r.parser.defined.push(i), e(d)) : n(d);
  }
}
function Bh(t, e, n) {
  return r;
  function r(o) {
    return rt(o) ? bn(t, i)(o) : n(o);
  }
  function i(o) {
    return $s(
      t,
...
function jh(t, e, n) {
  ...
  function i(a) {
    return K(a) ? (t.exit(f.hardBreakEscape), e(a)) : n(a);
  }
}
const Vh = {
  name: "headingAtx",
...
function Wh(t, e, n) {
  ...
  return i;
  function i(l) {
    return t.enter(f.atxHeading), a(l);
  }
  function a(l) {
    return x(l === m.numberSign, "expected `#`"), t.enter(f.atxHeadingSequence), s(l);
  }
  function s(l) {
    return l === m.numberSign && r++ < j.atxHeadingOpeningFenceSizeMax ? (t.consume(l), s) : l === m.eof || rt(l) ? (t.exit(f.atxHeadingSequence), o(l)) : n(l);
  }
  function o(l) {
    return l === m.numberSign ? (t.enter(f.atxHeadingSequence), c(l)) : l === m.eof || K(l) ? (t.exit(f.atxHeading), e(l)) : Ce(l) ? ve(t, o, f.whitespace)(l) : (t.enter(f.atxHeadingText), u(l));
  }
  function c(l) {
    return l === m.numberSign ? (t.consume(l), c) : (t.exit(f.atxHeadingSequence), o(l));
  }
  function u(l) {
    return l === m.eof || l === m.numberSign || rt(l) ? (t.exit(f.atxHeadingText), o(l)) : (t.consume(l), u);
  }
}
const qh = [
  "address",
  ...
  "track",
  "ul"
], ka = ["pre", "script", "style", "textarea"], Kh = {
  concrete: !0,
  name: "htmlFlow",
  resolveTo: Xh,
  tokenize: Jh
}, Zh = { partial: !0, tokenize: ed }, Yh = {
  partial: !0,
  tokenize: Qh
};
function Xh(t) {
...
function Jh(t, e, n) {
  ...
  function l(y) {
    return x(y === m.lessThan, "expected `<`"), t.enter(f.htmlFlow), t.enter(f.htmlFlowData), t.consume(y), h;
  }
  function h(y) {
    return y === m.exclamationMark ? (t.consume(y), g) : y === m.slash ? (t.consume(y), a = !0, k) : y === m.questionMark ? (t.consume(y), i = j.htmlInstruction, r.interrupt ? e : w) : bt(y) ? (x(y !== null), t.consume(y), s = String.fromCharCode(y), I) : n(y);
  }
  function g(y) {
    return y === m.dash ? (t.consume(y), i = j.htmlComment, d) : y === m.leftSquareBracket ? (t.consume(y), i = j.htmlCdata, o = 0, C) : bt(y) ? (t.consume(y), i = j.htmlDeclaration, r.interrupt ? e : w) : n(y);
  }
  function d(y) {
    return y === m.dash ? (t.consume(y), r.interrupt ? e : w) : n(y);
  }
  function C(y) {
    const ue = j.cdataOpeningString;
    return y === ue.charCodeAt(o++) ? (t.consume(y), o === ue.length ? r.interrupt ? e : D : C) : n(y);
  }
  function k(y) {
    return bt(y) ? (x(y !== null), t.consume(y), s = String.fromCharCode(y), I) : n(y);
  }
  function I(y) {
    if (y === m.eof || y === m.slash || y === m.greaterThan || rt(y)) {
      const ue = y === m.slash, $ = s.toLowerCase();
      return !ue && !a && ka.includes($) ? (i = j.htmlRaw, r.interrupt ? e(y) : D(y)) : qh.includes(s.toLowerCase()) ? (i = j.htmlBasic, ue ? (t.consume(y), T) : r.interrupt ? e(y) : D(y)) : (i = j.htmlComplete, r.interrupt && !r.parser.lazy[r.now().line] ? n(y) : a ? b(y) : v(y));
    }
    return y === m.dash || pt(y) ? (t.consume(y), s += String.fromCharCode(y), I) : n(y);
  }
  function T(y) {
    return y === m.greaterThan ? (t.consume(y), r.interrupt ? e : D) : n(y);
  }
  function b(y) {
    return Ce(y) ? (t.consume(y), b) : Z(y);
  }
  function v(y) {
    return y === m.slash ? (t.consume(y), Z) : y === m.colon || y === m.underscore || bt(y) ? (t.consume(y), O) : Ce(y) ? (t.consume(y), v) : Z(y);
  }
  function O(y) {
    return y === m.dash || y === m.dot || y === m.colon || y === m.underscore || pt(y) ? (t.consume(y), O) : A(y);
  }
  function A(y) {
    return y === m.equalsTo ? (t.consume(y), E) : Ce(y) ? (t.consume(y), A) : v(y);
  }
  function E(y) {
    return y === m.eof || y === m.lessThan || y === m.equalsTo || y === m.greaterThan || y === m.graveAccent ? n(y) : y === m.quotationMark || y === m.apostrophe ? (t.consume(y), c = y, U) : Ce(y) ? (t.consume(y), E) : B(y);
  }
  function U(y) {
    return y === c ? (t.consume(y), c = null, G) : y === m.eof || K(y) ? n(y) : (t.consume(y), U);
  }
  function B(y) {
    return y === m.eof || y === m.quotationMark || y === m.apostrophe || y === m.slash || y === m.lessThan || y === m.equalsTo || y === m.greaterThan || y === m.graveAccent || rt(y) ? A(y) : (t.consume(y), B);
  }
  function G(y) {
    return y === m.slash || y === m.greaterThan || Ce(y) ? v(y) : n(y);
  }
  function Z(y) {
    return y === m.greaterThan ? (t.consume(y), se) : n(y);
  }
  function se(y) {
    return y === m.eof || K(y) ? D(y) : Ce(y) ? (t.consume(y), se) : n(y);
  }
  function D(y) {
    return y === m.dash && i === j.htmlComment ? (t.consume(y), q) : y === m.lessThan && i === j.htmlRaw ? (t.consume(y), te) : y === m.greaterThan && i === j.htmlDeclaration ? (t.consume(y), Y) : y === m.questionMark && i === j.htmlInstruction ? (t.consume(y), w) : y === m.rightSquareBracket && i === j.htmlCdata ? (t.consume(y), Se) : K(y) && (i === j.htmlBasic || i === j.htmlComplete) ? (t.exit(f.htmlFlowData), t.check(
      Zh,
      W,
      M
    )(y)) : y === m.eof || K(y) ? (t.exit(f.htmlFlowData), M(y)) : (t.consume(y), D);
  }
  function M(y) {
    return t.check(
      Yh,
  ...
  function N(y) {
    return x(K(y)), t.enter(f.lineEnding), t.consume(y), t.exit(f.lineEnding), z;
  }
  function z(y) {
    return y === m.eof || K(y) ? M(y) : (t.enter(f.htmlFlowData), D(y));
  }
  function q(y) {
    return y === m.dash ? (t.consume(y), w) : D(y);
  }
  function te(y) {
    return y === m.slash ? (t.consume(y), s = "", ge) : D(y);
  }
  function ge(y) {
    if (y === m.greaterThan) {
      const ue = s.toLowerCase();
      return ka.includes(ue) ? (t.consume(y), Y) : D(y);
    }
    return bt(y) && s.length < j.htmlRawSizeMax ? (x(y !== null), t.consume(y), s += String.fromCharCode(y), ge) : D(y);
  }
  function Se(y) {
    return y === m.rightSquareBracket ? (t.consume(y), w) : D(y);
  }
  function w(y) {
    return y === m.greaterThan ? (t.consume(y), Y) : y === m.dash && i === j.htmlComment ? (t.consume(y), w) : D(y);
  }
  function Y(y) {
    return y === m.eof || K(y) ? (t.exit(f.htmlFlowData), W(y)) : (t.consume(y), Y);
  }
  function W(y) {
...
function Qh(t, e, n) {
  ...
  return i;
  function i(s) {
    return K(s) ? (t.enter(f.lineEnding), t.consume(s), t.exit(f.lineEnding), a) : n(s);
  }
  function a(s) {
    return r.parser.lazy[r.now().line] ? n(s) : e(s);
  }
...
function ed(t, e, n) {
  return r;
  function r(i) {
    return x(K(i), "expected a line ending"), t.enter(f.lineEnding), t.consume(i), t.exit(f.lineEnding), t.attempt(sr, e, n);
  }
}
const td = { name: "htmlText", tokenize: nd };
function nd(t, e, n) {
  const r = this;
  let i, a, s;
  return o;
  function o(w) {
    return x(w === m.lessThan, "expected `<`"), t.enter(f.htmlText), t.enter(f.htmlTextData), t.consume(w), c;
  }
  function c(w) {
    return w === m.exclamationMark ? (t.consume(w), u) : w === m.slash ? (t.consume(w), A) : w === m.questionMark ? (t.consume(w), v) : bt(w) ? (t.consume(w), B) : n(w);
  }
  function u(w) {
    return w === m.dash ? (t.consume(w), l) : w === m.leftSquareBracket ? (t.consume(w), a = 0, C) : bt(w) ? (t.consume(w), b) : n(w);
  }
  function l(w) {
    return w === m.dash ? (t.consume(w), d) : n(w);
  }
  function h(w) {
    return w === m.eof ? n(w) : w === m.dash ? (t.consume(w), g) : K(w) ? (s = h, te(w)) : (t.consume(w), h);
  }
  function g(w) {
    return w === m.dash ? (t.consume(w), d) : h(w);
  }
  function d(w) {
    return w === m.greaterThan ? q(w) : w === m.dash ? g(w) : h(w);
  }
  function C(w) {
    const Y = j.cdataOpeningString;
    return w === Y.charCodeAt(a++) ? (t.consume(w), a === Y.length ? k : C) : n(w);
  }
  function k(w) {
    return w === m.eof ? n(w) : w === m.rightSquareBracket ? (t.consume(w), I) : K(w) ? (s = k, te(w)) : (t.consume(w), k);
  }
  function I(w) {
    return w === m.rightSquareBracket ? (t.consume(w), T) : k(w);
  }
  function T(w) {
    return w === m.greaterThan ? q(w) : w === m.rightSquareBracket ? (t.consume(w), T) : k(w);
  }
  function b(w) {
    return w === m.eof || w === m.greaterThan ? q(w) : K(w) ? (s = b, te(w)) : (t.consume(w), b);
  }
  function v(w) {
    return w === m.eof ? n(w) : w === m.questionMark ? (t.consume(w), O) : K(w) ? (s = v, te(w)) : (t.consume(w), v);
  }
  function O(w) {
    return w === m.greaterThan ? q(w) : v(w);
  }
  function A(w) {
    return bt(w) ? (t.consume(w), E) : n(w);
  }
  function E(w) {
    return w === m.dash || pt(w) ? (t.consume(w), E) : U(w);
  }
  function U(w) {
    return K(w) ? (s = U, te(w)) : Ce(w) ? (t.consume(w), U) : q(w);
  }
  function B(w) {
    return w === m.dash || pt(w) ? (t.consume(w), B) : w === m.slash || w === m.greaterThan || rt(w) ? G(w) : n(w);
  }
  function G(w) {
    return w === m.slash ? (t.consume(w), q) : w === m.colon || w === m.underscore || bt(w) ? (t.consume(w), Z) : K(w) ? (s = G, te(w)) : Ce(w) ? (t.consume(w), G) : q(w);
  }
  function Z(w) {
    return w === m.dash || w === m.dot || w === m.colon || w === m.underscore || pt(w) ? (t.consume(w), Z) : se(w);
  }
  function se(w) {
    return w === m.equalsTo ? (t.consume(w), D) : K(w) ? (s = se, te(w)) : Ce(w) ? (t.consume(w), se) : G(w);
  }
  function D(w) {
    return w === m.eof || w === m.lessThan || w === m.equalsTo || w === m.greaterThan || w === m.graveAccent ? n(w) : w === m.quotationMark || w === m.apostrophe ? (t.consume(w), i = w, M) : K(w) ? (s = D, te(w)) : Ce(w) ? (t.consume(w), D) : (t.consume(w), N);
  }
  function M(w) {
    return w === i ? (t.consume(w), i = void 0, z) : w === m.eof ? n(w) : K(w) ? (s = M, te(w)) : (t.consume(w), M);
  }
  function N(w) {
    return w === m.eof || w === m.quotationMark || w === m.apostrophe || w === m.lessThan || w === m.equalsTo || w === m.graveAccent ? n(w) : w === m.slash || w === m.greaterThan || rt(w) ? G(w) : (t.consume(w), N);
  }
  function z(w) {
    return w === m.slash || w === m.greaterThan || rt(w) ? G(w) : n(w);
  }
  function q(w) {
    return w === m.greaterThan ? (t.consume(w), t.exit(f.htmlTextData), t.exit(f.htmlText), e) : n(w);
  }
  function te(w) {
    return x(s, "expected return state"), x(K(w), "expected eol"), t.exit(f.htmlTextData), t.enter(f.lineEnding), t.consume(w), t.exit(f.lineEnding), ge;
  }
  function ge(w) {
    return x(
      r.parser.constructs.disable.null,
...
const yi = {
  ...
  resolveTo: od,
  tokenize: ld
}, rd = { tokenize: cd }, id = { tokenize: ud }, ad = { tokenize: hd };
function sd(t) {
  let e = -1;
  const n = [];
  for (; ++e < t.length; ) {
...
function od(t, e) {
  ...
  return o = [
    ["enter", c, e],
    ["enter", u, e]
  ], o = gt(o, t.slice(a + 1, a + r + 3)), o = gt(o, [["enter", l, e]]), x(
    e.parser.constructs.insideSpan.null,
    ...
    t[s - 1],
    ["exit", u, e]
  ]), o = gt(o, t.slice(s + 1)), o = gt(o, [["exit", c, e]]), vt(t, a, t.length, o), t;
}
function ld(t, e, n) {
  const r = this;
  ...
  return o;
  function o(g) {
    return x(g === m.rightSquareBracket, "expected `]`"), a ? a._inactive ? h(g) : (s = r.parser.defined.includes(
      on(
        r.sliceSerialize({ start: a.end, end: r.now() })
      )
    ), t.enter(f.labelEnd), t.enter(f.labelMarker), t.consume(g), t.exit(f.labelMarker), t.exit(f.labelEnd), c) : n(g);
  }
  function c(g) {
...
function cd(t, e, n) {
  return r;
  function r(h) {
    return x(h === m.leftParenthesis, "expected left paren"), t.enter(f.resource), t.enter(f.resourceMarker), t.consume(h), t.exit(f.resourceMarker), i;
  }
  function i(h) {
    return rt(h) ? bn(t, a)(h) : a(h);
  }
  function a(h) {
    return h === m.rightParenthesis ? l(h) : Us(
      t,
      s,
      o,
  ...
  function s(h) {
    return rt(h) ? bn(t, c)(h) : l(h);
  }
  function o(h) {
    return n(h);
  }
  function c(h) {
    return h === m.quotationMark || h === m.apostrophe || h === m.leftParenthesis ? $s(
      t,
      u,
      n,
      f.resourceTitle,
  ...
  function u(h) {
    return rt(h) ? bn(t, l)(h) : l(h);
  }
  function l(h) {
    return h === m.rightParenthesis ? (t.enter(f.resourceMarker), t.consume(h), t.exit(f.resourceMarker), t.exit(f.resource), e) : n(h);
  }
}
function ud(t, e, n) {
  const r = this;
  return i;
  function i(o) {
    return x(o === m.leftSquareBracket, "expected left bracket"), Bs.call(
      r,
      t,
      a,
      s,
...
const _r = { name: "lineEnding", tokenize: md };
function md(t, e) {
  return n;
  function n(r) {
    return x(K(r), "expected eol"), t.enter(f.lineEnding), t.consume(r), t.exit(f.lineEnding), ve(t, e, f.linePrefix);
  }
}
const Zn = {
  name: "thematicBreak",
...
function yd(t, e, n) {
  ...
  function o(u) {
    return u === i ? (t.enter(f.thematicBreakSequence), c(u)) : r >= j.thematicBreakMarkerCountMin && (u === m.eof || K(u)) ? (t.exit(f.thematicBreak), e(u)) : n(u);
  }
  function c(u) {
    return u === i ? (t.consume(u), r++, c) : (t.exit(f.thematicBreakSequence), Ce(u) ? ve(t, o, f.whitespace)(u) : o(u));
  }
}
const nt = {
  continuation: { tokenize: xd },
  exit: Td,
  name: "list",
  tokenize: Sd
}, Cd = {
...
function Sd(t, e, n) {
  ...
  function o(d) {
    ...
    const C = r.containerState.type || (d === m.asterisk || d === m.plusSign || d === m.dash ? f.listUnordered : f.listOrdered);
    if (C === f.listUnordered ? !r.containerState.marker || d === r.containerState.marker : Xr(d)) {
      if (r.containerState.type || (r.containerState.type = C, t.enter(C, { _container: !0 })), C === f.listUnordered)
        return t.enter(f.listItemPrefix), d === m.asterisk || d === m.dash ? t.check(Zn, n, u)(d) : u(d);
      if (!r.interrupt || d === m.digit1)
        return t.enter(f.listItemPrefix), t.enter(f.listItemValue), c(d);
    }
    return n(d);
  }
  function c(d) {
    return x(r.containerState, "expected state"), Xr(d) && ++s < j.listItemValueSizeMax ? (t.consume(d), c) : (!r.interrupt || s < 2) && (r.containerState.marker ? d === r.containerState.marker : d === m.rightParenthesis || d === m.dot) ? (t.exit(f.listItemValue), u(d)) : n(d);
  }
  function u(d) {
    return x(r.containerState, "expected state"), x(d !== m.eof, "eof (`null`) is not a marker"), t.enter(f.listItemMarker), t.consume(d), t.exit(f.listItemMarker), r.containerState.marker = r.containerState.marker || d, t.check(
      sr,
  ...
  function l(d) {
    return x(r.containerState, "expected state"), r.containerState.initialBlankLine = !0, a++, g(d);
  }
  function h(d) {
    return Ce(d) ? (t.enter(f.listItemPrefixWhitespace), t.consume(d), t.exit(f.listItemPrefixWhitespace), g) : n(d);
  }
  function g(d) {
    return x(r.containerState, "expected state"), r.containerState.size = a + r.sliceSerialize(t.exit(f.listItemPrefix), !0).length, e(d);
  }
}
function xd(t, e, n) {
  const r = this;
  return x(r.containerState, "expected state"), r.containerState._closeFlow = void 0, t.check(sr, i, a);
  function i(o) {
    return x(r.containerState, "expected state"), x(typeof r.containerState.size == "number", "expected size"), r.containerState.furtherBlankLines = r.containerState.furtherBlankLines || r.containerState.initialBlankLine, ve(
      t,
      e,
      f.listItemIndent,
  ...
  function a(o) {
    return x(r.containerState, "expected state"), r.containerState.furtherBlankLines || !Ce(o) ? (r.containerState.furtherBlankLines = void 0, r.containerState.initialBlankLine = void 0, s(o)) : (r.containerState.furtherBlankLines = void 0, r.containerState.initialBlankLine = void 0, t.attempt(wd, e, s)(o));
  }
  function s(o) {
...
function kd(t, e, n) {
  const r = this;
  return x(r.containerState, "expected state"), x(typeof r.containerState.size == "number", "expected size"), ve(
    t,
    i,
    f.listItemIndent,
    r.containerState.size + 1
...
function Ed(t, e, n) {
  ...
  function i(a) {
    const s = r.events[r.events.length - 1];
    return !Ce(a) && s && s[1].type === f.listItemPrefixWhitespace ? e(a) : n(a);
  }
}
const Ta = {
...
function _d(t, e, n) {
  ...
  function a(u) {
    ...
    for (x(
      ...
      if (r.events[l][1].type !== f.lineEnding && r.events[l][1].type !== f.linePrefix && r.events[l][1].type !== f.content) {
        h = r.events[l][1].type === f.paragraph;
        break;
      }
    return !r.parser.lazy[r.now().line] && (r.interrupt || h) ? (t.enter(f.setextHeadingLine), i = u, s(u)) : n(u);
  }
  function s(u) {
    return t.enter(f.setextHeadingLineSequence), o(u);
  }
  function o(u) {
    return u === i ? (t.consume(u), o) : (t.exit(f.setextHeadingLineSequence), Ce(u) ? ve(t, c, f.lineSuffix)(u) : c(u));
  }
  function c(u) {
    return u === m.eof || K(u) ? (t.exit(f.setextHeadingLine), e(u)) : n(u);
  }
}
const vd = { tokenize: Rd };
function Rd(t) {
...
const Id = { resolveAll: Vs() }, Md = js("string"), Nd = js("text");
function js(t) {
  return {
    resolveAll: Vs(
      t === "text" ? Ad : void 0
  ...
  function e(n) {
    ...
    function o(l) {
      ...
      return n.enter(f.data), n.consume(l), c;
    }
    function c(l) {
      return u(l) ? (n.exit(f.data), a(l)) : (n.consume(l), c);
    }
    function u(l) {
      if (l === m.eof)
        return !0;
      ...
      let g = -1;
      if (h)
        for (x(Array.isArray(h), "expected `disable.null` to be populated"); ++g < h.length; ) {
          const d = h[g];
  ...
  for (; ++n <= t.length; )
    if ((n === t.length || t[n][1].type === f.lineEnding) && t[n - 1][1].type === f.data) {
      ...
      if (e._contentTypeTextTrailing && n === t.length && (o = 0), o) {
        const u = {
          type: n === t.length || c || o < j.hardBreakPrefixSizeMin ? f.lineSuffix : f.hardBreakTrailing,
          start: {
...
const Ld = {
  ...
  [m.carriageReturn]: _r,
  [m.lineFeed]: _r,
  [m.carriageReturnLineFeed]: _r,
  [m.exclamationMark]: dd,
  [m.ampersand]: Fs,
  [m.asterisk]: Jr,
  [m.lessThan]: [gh, td],
  [m.leftSquareBracket]: fd,
  [m.backslash]: [$h, Ds],
  [m.rightSquareBracket]: yi,
  [m.underscore]: Jr,
  [m.graveAccent]: Rh
}, zd = { null: [Jr, Id] }, Ud = { null: [m.asterisk, m.underscore] }, Bd = { null: [] }, $d = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  attentionMarkers: Ud,
  contentInitial: Od,
  disable: Bd,
...
function jd() {
  ...
  function s(l) {
    if (l = String(l), !(l.length > 100)) {
      ...
      if (h) {
        var g = parseFloat(h[1]), d = (h[2] || "ms").toLowerCase();
        switch (d) {
          case "years":
          case "year":
          case "yrs":
  ...
  function u(l, h, g, d) {
    var C = h >= g * 1.5;
    return Math.round(l / g) + " " + d + (C ? "s" : "");
  }
  return vr;
}
function Vd(t) {
  n.debug = n, n.default = n, n.coerce = c, n.disable = s, n.enable = i, n.enabled = o, n.humanize = jd(), n.destroy = u, Object.keys(t).forEach((l) => {
    n[l] = t[l];
  ...
  function n(l) {
    ...
    function k(...I) {
      if (!k.enabled)
        return;
      const T = k, b = Number(/* @__PURE__ */ new Date()), v = b - (h || b);
      T.diff = v, T.prev = h, T.curr = b, h = b, I[0] = n.coerce(I[0]), typeof I[0] != "string" && I.unshift("%O");
      let O = 0;
      I[0] = I[0].replace(/%([a-zA-Z%])/g, (E, U) => {
        ...
        if (typeof B == "function") {
          const G = I[O];
          E = B.call(T, G), I.splice(O, 1), O--;
        }
        return E;
      }), n.formatArgs.call(T, I), (T.log || n.log).apply(T, I);
    }
    return k.namespace = l, k.useColors = n.useColors(), k.color = n.selectColor(l), k.extend = r, k.destroy = n.destroy, Object.defineProperty(k, "enabled", {
      enumerable: !0,
      configurable: !1,
      get: () => g !== null ? g : (d !== n.namespaces && (d = n.namespaces, C = n.enabled(l)), C),
      set: (I) => {
        g = I;
      }
  ...
  function r(l, h) {
    const g = n(this.namespace + (typeof h > "u" ? ":" : h) + l);
...
var Wd = Qr.exports;
const qd = /* @__PURE__ */ oi(Wd), Kt = qd("micromark");
function Kd(t, e, n) {
  let r = {
    _bufferIndex: -1,
    _index: 0,
  ...
  let h = e.tokenize.call(l, u), g;
  return e.resolveAll && a.push(e), l;
  function d(M) {
    return s = gt(s, M), b(), s[s.length - 1] !== m.eof ? [] : (Z(e, 0), l.events = mi(a, l.events, l), l.events);
  }
  function C(M, N) {
    return Yd(k(M), N);
  }
  ...
  function I() {
    const { _bufferIndex: M, _index: N, line: z, column: q, offset: te } = r;
    return { _bufferIndex: M, _index: N, line: z, column: q, offset: te };
  }
  function T(M) {
    i[M.line] = M.column, D(), Kt("position: define skip: `%j`", r);
  }
  function b() {
    let M;
    for (; r._index < s.length; ) {
      const N = s[r._index];
      if (typeof N == "string")
        for (M = r._index, r._bufferIndex < 0 && (r._bufferIndex = 0); r._index === M && r._bufferIndex < N.length; )
  ...
  function v(M) {
    x(c === !0, "expected character to be consumed"), c = void 0, Kt("main: passing `%s` to %s", M, h && h.name), g = M, x(typeof h == "function", "expected state"), h = h(M);
  }
  function O(M) {
    x(M === g, "expected given code to equal expected code"), Kt("consume: `%s`", M), x(
      c === void 0,
      "expected code to not have been consumed: this might be because `return x(code)` instead of `return x` was used"
    ), x(
      M === null ? l.events.length === 0 || l.events[l.events.length - 1][0] === "exit" : l.events[l.events.length - 1][0] === "enter",
      "expected last token to be open"
    ), K(M) ? (r.line++, r.column = 1, r.offset += M === m.carriageReturnLineFeed ? 2 : 1, D(), Kt("position: after eol: `%j`", r)) : M !== m.virtualSpace && (r.column++, r.offset++), r._bufferIndex < 0 ? r._index++ : (r._bufferIndex++, r._bufferIndex === // Points w/ non-negative `_bufferIndex` reference
    // strings.
    /** @type {string} */
    s[r._index].length && (r._bufferIndex = -1, r._index++)), l.previous = M, c = !0;
  }
  function A(M, N) {
    const z = N || {};
    return z.type = M, z.start = I(), x(typeof M == "string", "expected string type"), x(M.length > 0, "expected non-empty string"), Kt("enter: `%s`", M), l.events.push(["enter", z, l]), o.push(z), z;
  }
  function E(M) {
    x(typeof M == "string", "expected string type"), x(M.length > 0, "expected non-empty string");
    const N = o.pop();
    return x(N, "cannot close w/o open tokens"), N.end = I(), x(M === N.type, "expected exit token to match current token"), x(
      !(N.start._index === N.end._index && N.start._bufferIndex === N.end._bufferIndex),
      "expected non-empty token (`" + M + "`)"
    ), Kt("exit: `%s`", N.type), l.events.push(["exit", N, l]), N;
  }
  function U(M, N) {
    Z(M, N.from);
  }
  ...
  function G(M, N) {
    ...
    function z(q, te, ge) {
      ...
      function y(Q) {
        return Te;
        function Te(me) {
          const ye = me !== null && Q[me], Ie = me !== null && Q.null, Ue = [
      ...
      function ue(Q) {
        return Se = Q, w = 0, Q.length === 0 ? (x(ge, "expected `bogusState` to be given"), ge) : $(Q[w]);
      }
      function $(Q) {
        return Te;
        function Te(me) {
          return W = se(), Y = Q, Q.partial || (l.currentConstruct = Q), x(
            l.parser.constructs.disable.null,
            "expected `disable.null` to be populated"
          ), Q.name && l.parser.constructs.disable.null.includes(Q.name) ? re(me) : Q.tokenize.call(
      ...
      function oe(Q) {
        return x(Q === g, "expected code"), c = !0, M(Y, W), te;
      }
      function re(Q) {
        return x(Q === g, "expected code"), c = !0, W.restore(), ++w < Se.length ? $(Se[w]) : ge;
      }
    }
  }
  function Z(M, N) {
    M.resolveAll && !a.includes(M) && a.push(M), M.resolve && vt(
      ...
      N,
      l.events.length - N,
      M.resolve(l.events.slice(N), l)
    ), M.resolveTo && (l.events = M.resolveTo(l.events, l)), x(
      M.partial || l.events.length === 0 || l.events[l.events.length - 1][0] === "exit",
      "expected last token to end"
    );
  }
  function se() {
    const M = I(), N = l.previous, z = l.currentConstruct, q = l.events.length, te = Array.from(o);
    return { from: q, restore: ge };
    function ge() {
      r = M, l.previous = N, l.currentConstruct = z, l.events.length = q, o = te, D(), Kt("position: restore: `%j`", r);
    }
  }
  function D() {
...
function Qd() {
  ...
  function i(a, s, o) {
    const c = [];
    let u, l, h, g, d;
    for (a = e + (typeof a == "string" ? a.toString() : new TextDecoder(s || void 0).decode(a)), h = 0, e = "", n && (a.charCodeAt(0) === m.byteOrderMarker && h++, n = void 0); h < a.length; ) {
      if (ba.lastIndex = h, u = ba.exec(a), g = u && u.index !== void 0 ? u.index : a.length, d = a.charCodeAt(g), !u) {
        e = a.slice(h);
        break;
      }
      if (d === m.lf && h === g && r)
        c.push(m.carriageReturnLineFeed), r = void 0;
      else
        switch (r && (c.push(m.carriageReturn), r = void 0), h < g && (c.push(a.slice(h, g)), t += g - h), d) {
          case m.nul: {
            c.push(m.replacementCharacter), t++;
            break;
          }
          case m.ht: {
            for (l = Math.ceil(t / j.tabSize) * j.tabSize, c.push(m.horizontalTab); t++ < l; ) c.push(m.virtualSpace);
            break;
          }
          case m.lf: {
            c.push(m.lineFeed), t = 1;
            break;
          }
          default:
            r = !0, t = 1;
    ...
    return o && (r && c.push(m.carriageReturn), e && c.push(e), c.push(m.eof)), c;
  }
}
const e1 = /\\([!-/:-@[-`{-~])|&(#(?:\d{1,7}|x[\da-f]{1,6})|[\da-z]{1,31});/gi;
function t1(t) {
  return t.replace(e1, n1);
}
function n1(t, e, n) {
  if (e)
    return e;
...
function r1(t, e, n) {
  return typeof e != "string" && (n = e, e = void 0), i1(n)(
    Jd(
      Xd(n).document().write(Qd()(t, e, !0))
    )
  );
}
function i1(t) {
  ...
  function r(_) {
    ...
    let ne = -1;
    for (; ++ne < _.length; )
      if (_[ne][1].type === f.listOrdered || _[ne][1].type === f.listUnordered)
        if (_[ne][0] === "enter")
          he.push(ne);
        else {
          const Ee = he.pop();
          x(typeof Ee == "number", "expected list ot be open"), ne = i(_, Ee, ne);
        }
    for (ne = -1; ++ne < _.length; ) {
      const Ee = e[_[ne][0]];
      Gs.call(Ee, _[ne][1].type) && Ee[_[ne][1].type].call(
        Object.assign(
          { sliceSerialize: _[ne][2].sliceSerialize },
          V
        ),
        _[ne][1]
      );
    ...
    if (V.tokenStack.length > 0) {
      const Ee = V.tokenStack[V.tokenStack.length - 1];
    ...
    for (R.position = {
      ...
      R = e.transforms[ne](R) || R;
    return R;
  }
  function i(_, R, V) {
    let he = R - 1, ne = -1, Ee = !1, lt, Le, ct, Be;
    for (; ++he <= V; ) {
      const Ae = _[he];
      switch (Ae[1].type) {
        ...
        case f.listOrdered:
        case f.blockQuote: {
          Ae[0] === "enter" ? ne++ : ne--, Be = void 0;
          break;
        }
        case f.lineEndingBlank: {
          Ae[0] === "enter" && (lt && !Be && !ne && !ct && (ct = he), Be = void 0);
          break;
        }
        case f.linePrefix:
      ...
      if (!ne && Ae[0] === "enter" && Ae[1].type === f.listItemPrefix || ne === -1 && Ae[0] === "exit" && (Ae[1].type === f.listUnordered || Ae[1].type === f.listOrdered)) {
        if (lt) {
          let Ve = he;
          for (Le = void 0; Ve--; ) {
            const $e = _[Ve];
            if ($e[1].type === f.lineEnding || $e[1].type === f.lineEndingBlank) {
              if ($e[0] === "exit") continue;
              Le && (_[Le][1].type = f.lineEndingBlank, Ee = !0), $e[1].type = f.lineEnding, Le = Ve;
            } else if (!($e[1].type === f.linePrefix || $e[1].type === f.blockQuotePrefix || $e[1].type === f.blockQuotePrefixWhitespace || $e[1].type === f.blockQuoteMarker || $e[1].type === f.listItemIndent)) break;
          }
          ct && (!Le || ct < Le) && (lt._spread = !0), lt.end = Object.assign(
            {},
            Le ? _[Le][1].start : Ae[1].end
          ), _.splice(Le || he, 0, ["exit", lt, Ae[2]]), he++, V++;
        }
        if (Ae[1].type === f.listItemPrefix) {
          const Ve = {
            type: "listItem",
            _spread: !1,
          ...
          lt = Ve, _.splice(he, 0, ["enter", Ve, Ae[2]]), he++, V++, ct = void 0, Be = !0;
        }
      }
    }
    return _[R][1]._spread = Ee, V;
  }
  function a(_, R) {
    return V;
    function V(he) {
      o.call(this, _(he), he), R && R.call(this, he);
    }
  }
  function s() {
  ...
  function o(_, R, V) {
    ...
    x(he, "expected `parent`"), x("children" in he, "expected `parent`"), he.children.push(_), this.stack.push(_), this.tokenStack.push([R, V || void 0]), _.position = {
      start: Bt(R.start),
  ...
  function c(_) {
    return R;
    function R(V) {
      _ && _.call(this, V), u.call(this, V);
    }
  }
  function u(_, R) {
    const V = this.stack.pop();
    x(V, "expected `node`");
    const he = this.tokenStack.pop();
    if (he)
      he[0].type !== _.type && (R ? R.call(this, _, he[0]) : (he[1] || _a).call(this, _, he[0]));
    else throw new Error(
      "Cannot close `" + _.type + "` (" + En({ start: _.start, end: _.end }) + "): it’s not open"
    );
    x(V.type !== "fragment", "unexpected fragment `exit`ed"), x(V.position, "expected `position` to be defined"), V.position.end = Bt(_.end);
  }
  function l() {
  ...
  function g(_) {
    if (this.data.expectingFirstListItemValue) {
      ...
      x(R, "expected nodes on stack"), x(R.type === "list", "expected list on stack"), R.start = Number.parseInt(
  ...
  function d() {
    const _ = this.resume(), R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "code", "expected code on stack"), R.lang = _;
  }
  function C() {
    const _ = this.resume(), R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "code", "expected code on stack"), R.meta = _;
  }
  function k() {
  ...
  function I() {
    const _ = this.resume(), R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "code", "expected code on stack"), R.value = _.replace(/^(\r?\n|\r)|(\r?\n|\r)$/g, ""), this.data.flowCodeInside = void 0;
  }
  function T() {
    const _ = this.resume(), R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "code", "expected code on stack"), R.value = _.replace(/(\r?\n|\r)$/g, "");
  ...
  function b(_) {
    ...
    x(V, "expected node on stack"), x(V.type === "definition", "expected definition on stack"), V.label = R, V.identifier = on(
      this.sliceSerialize(_)
    ).toLowerCase();
  }
  function v() {
    const _ = this.resume(), R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "definition", "expected definition on stack"), R.title = _;
  }
  function O() {
    const _ = this.resume(), R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "definition", "expected definition on stack"), R.url = _;
  }
  function A(_) {
    const R = this.stack[this.stack.length - 1];
    if (x(R, "expected node on stack"), x(R.type === "heading", "expected heading on stack"), !R.depth) {
      const V = this.sliceSerialize(_).length;
      x(
  ...
  function U(_) {
    const R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "heading", "expected heading on stack"), R.depth = this.sliceSerialize(_).codePointAt(0) === m.equalsTo ? 1 : 2;
  }
  function B() {
  ...
  function G(_) {
    const R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x("children" in R, "expected parent on stack");
    const V = R.children;
    let he = V[V.length - 1];
    (!he || he.type !== "text") && (he = At(), he.position = {
      start: Bt(_.start),
      // @ts-expect-error: we’ll add `end` later.
      end: void 0
    }, V.push(he)), this.stack.push(he);
  }
  function Z(_) {
    const R = this.stack.pop();
    x(R, "expected a `node` to be on the stack"), x("value" in R, "expected a `literal` to be on the stack"), x(R.position, "expected `node` to have an open position"), R.value += this.sliceSerialize(_), R.position.end = Bt(_.end);
  }
  function se(_) {
    const R = this.stack[this.stack.length - 1];
    if (x(R, "expected `node`"), this.data.atHardBreak) {
      x("children" in R, "expected `parent`");
      const V = R.children[R.children.length - 1];
      x(V.position, "expected tail to have a starting position"), V.position.end = Bt(_.end), this.data.atHardBreak = void 0;
      return;
    }
    !this.data.setextHeadingSlurpLineEnding && e.canContainEols.includes(R.type) && (G.call(this, _), Z.call(this, _));
  }
  function D() {
  ...
  function M() {
    const _ = this.resume(), R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "html", "expected html on stack"), R.value = _;
  }
  function N() {
    const _ = this.resume(), R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "html", "expected html on stack"), R.value = _;
  ...
  function z() {
    const _ = this.resume(), R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "inlineCode", "expected inline code on stack"), R.value = _;
  }
  function q() {
    ...
    if (x(_, "expected node on stack"), x(_.type === "link", "expected link on stack"), this.data.inReference) {
      ...
      _.type += "Reference", _.referenceType = R, delete _.url, delete _.title;
    } else
  ...
  function ge(_) {
    const R = this.sliceSerialize(_), V = this.stack[this.stack.length - 2];
    x(V, "expected ancestor on stack"), x(
      V.type === "image" || V.type === "link",
      "expected image or link on stack"
    ), V.label = t1(R), V.identifier = on(R).toLowerCase();
  }
  function Se() {
    const _ = this.stack[this.stack.length - 1];
    x(_, "expected node on stack"), x(_.type === "fragment", "expected fragment on stack");
    const R = this.resume(), V = this.stack[this.stack.length - 1];
    if (x(V, "expected node on stack"), x(
      V.type === "image" || V.type === "link",
      "expected image or link on stack"
  ...
  function oe(_) {
    const R = this.sliceSerialize(_), V = this.data.characterReferenceType;
    let he;
    if (V)
      he = Os(
        R,
        V === f.characterReferenceMarkerNumeric ? j.numericBaseDecimal : j.numericBaseHexadecimal
      ), this.data.characterReferenceType = void 0;
    else {
      const Ee = gi(R);
      x(Ee !== !1, "expected reference to decode"), he = Ee;
    }
    const ne = this.stack[this.stack.length - 1];
    x(ne, "expected `node`"), x("value" in ne, "expected `node.value`"), ne.value += he;
  }
  function re(_) {
    const R = this.stack.pop();
    x(R, "expected `node`"), x(R.position, "expected `node.position`"), R.position.end = Bt(_.end);
  }
  function Q(_) {
    Z.call(this, _);
    const R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "link", "expected link on stack"), R.url = this.sliceSerialize(_);
  }
  function Te(_) {
    Z.call(this, _);
    const R = this.stack[this.stack.length - 1];
    x(R, "expected node on stack"), x(R.type === "link", "expected link on stack"), R.url = "mailto:" + this.sliceSerialize(_);
  }
  function me() {
...
function _a(t, e) {
  throw t ? new Error(
    "Cannot close `" + t.type + "` (" + En({ start: t.start, end: t.end }) + "): a different token (`" + e.type + "`, " + En({ start: e.start, end: e.end }) + ") is open"
  ) : new Error(
    "Cannot close document, a token (`" + e.type + "`, " + En({ start: e.start, end: e.end }) + ") is still open"
  );
}
function s1(t) {
  const e = this;
...
function d1(t, e) {
  const n = typeof t.options.clobberPrefix == "string" ? t.options.clobberPrefix : "user-content-", r = String(e.identifier).toUpperCase(), i = hn(r.toLowerCase()), a = t.footnoteOrder.indexOf(r);
...
function S1(t, e, n) {
  const r = t.all(e), i = n ? x1(n) : Ks(e), a = {}, s = [];
...
function _1(t, e) {
  ...
  if (n.length > 0) {
    const s = {
      ...
      properties: {},
      children: t.wrap(n, !0)
    }, o = hi(e.children[1]), c = vs(e.children[e.children.length - 1]);
...
function v1(t, e, n) {
  ...
  const u = [];
  for (; ++c < o; ) {
    const h = e.children[c], g = {}, d = s ? s[c] : void 0;
    d && (g.align = d);
...
function M1(t, e) {
  const n = { type: "text", value: I1(String(e.value)) };
  return t.patch(e, n), t.applyData(e, n);
}
function N1(t, e) {
  const n = {
...
const Zs = -1, or = 0, _n = 1, nr = 2, Ci = 3, wi = 4, Si = 5, xi = 6, Ys = 7, Xs = 8, Ma = typeof self == "object" ? self : globalThis, L1 = (t, e) => {
  const n = (i, a) => (t.set(a, i), i), r = (i) => {
    ...
    return n(new Ma[a](s), i);
  };
  return r;
}, Na = (t) => L1(/* @__PURE__ */ new Map(), t)(0), rn = "", { toString: O1 } = {}, { keys: P1 } = Object, xn = (t) => {
  const e = typeof t;
  if (e !== "object" || !t)
    return [or, e];
  const n = O1.call(t).slice(8, -1);
  switch (n) {
    case "Array":
      return [_n, rn];
    case "Object":
  ...
  return n.includes("Array") ? [_n, n] : n.includes("Error") ? [Ys, n] : [nr, n];
}, Gn = ([t, e]) => t === or && (e === "function" || e === "symbol"), D1 = (t, e, n, r) => {
  const i = (s, o) => {
    const c = r.push(s) - 1;
    return n.set(o, c), c;
  }, a = (s) => {
    ...
    switch (o) {
      ...
      case Si: {
        const l = [], h = i([o, l], s);
        for (const [g, d] of s)
          (t || !(Gn(xn(g)) || Gn(xn(d)))) && l.push([a(g), a(d)]);
        return h;
      }
      case xi: {
        const l = [], h = i([o, l], s);
        for (const g of s)
          (t || !Gn(xn(g))) && l.push(a(g));
        return h;
      }
    }
    const { message: u } = s;
  ...
  const r = [];
  return D1(!(e || n), !!e, /* @__PURE__ */ new Map(), r)(t), r;
}, rr = typeof structuredClone == "function" ? (
  /* c8 ignore start */
  (t, e) => e && ("json" in e || "lossy" in e) ? Na(Aa(t, e)) : structuredClone(t)
) : (t, e) => Na(Aa(t, e));
function F1(t, e) {
...
function z1(t) {
  const e = typeof t.options.clobberPrefix == "string" ? t.options.clobberPrefix : "user-content-", n = t.options.footnoteBackContent || F1, r = t.options.footnoteBackLabel || H1, i = t.options.footnoteLabel || "Footnotes", a = t.options.footnoteLabelTagName || "h2", s = t.options.footnoteLabelProperties || {
    className: ["sr-only"]
  }, o = [];
  let c = -1;
  for (; ++c < t.footnoteOrder.length; ) {
    ...
    if (!u)
      continue;
    const l = t.all(u), h = String(u.identifier).toUpperCase(), g = hn(h.toLowerCase());
    let d = 0;
    const C = [], k = t.footnoteCounts.get(h);
    for (; k !== void 0 && ++d <= k; ) {
    ...
    if (I && I.type === "element" && I.tagName === "p") {
      ...
      b && b.type === "text" ? b.value += " " : I.children.push({ type: "text", value: " " }), I.children.push(...C);
    } else
      l.push(...C);
    const T = {
      type: "element",
...
const ki = (
  ...
  function(t) {
    ...
    if (typeof t == "function")
      return lr(t);
    if (typeof t == "object")
      return Array.isArray(t) ? U1(t) : B1(t);
    if (typeof t == "string")
      return $1(t);
...
function Qs(t, e, n, r) {
  ...
  function o(c, u, l) {
    ...
    function g() {
      let d = Js, C, k, I;
      if ((!e || a(c, u, l[l.length - 1] || void 0)) && (d = q1(n(c, l)), d[0] === La))
        return d;
      if ("children" in c && c.children) {
        ...
        if (T.children && d[0] !== W1)
          for (k = (r ? T.children.length : -1) + s, I = l.concat(T); k > -1 && k < T.children.length; ) {
            const b = T.children[k];
            if (C = o(b, k, I)(), C[0] === La)
              return C;
...
function q1(t) {
  return Array.isArray(t) ? t : typeof t == "number" ? [G1, t] : t == null ? Js : [t];
}
function eo(t, e, n, r) {
  let i, a, s;
  typeof e == "function" && typeof n != "function" ? (a = void 0, s = e, i = n) : (a = e, s = n, i = r), Qs(t, a, o, i);
  function o(c, u) {
    const l = u[u.length - 1], h = l ? l.children.indexOf(c) : void 0;
    return s(c, h, l);
  }
}
const ei = {}.hasOwnProperty, K1 = {};
function Z1(t, e) {
  const n = e || K1, r = /* @__PURE__ */ new Map(), i = /* @__PURE__ */ new Map(), a = /* @__PURE__ */ new Map(), s = { ...A1, ...n.handlers }, o = {
    all: u,
    applyData: X1,
    definitionById: r,
    footnoteById: i,
  ...
  return eo(t, function(l) {
    if (l.type === "definition" || l.type === "footnoteDefinition") {
      const h = l.type === "definition" ? r : i, g = String(l.identifier).toUpperCase();
      h.has(g) || h.set(g, l);
    }
  }), o;
  function c(l, h) {
    const g = l.type, d = o.handlers[g];
    if (ei.call(o.handlers, g) && d)
      return d(o, l, h);
    if (o.options.passThrough && o.options.passThrough.includes(g)) {
      if ("children" in l) {
        const { children: k, ...I } = l, T = rr(I);
        return T.children = o.all(l), T;
      }
      return rr(l);
    }
    return (o.options.unknownHandler || J1)(o, l, h);
  }
  function u(l) {
    const h = [];
    if ("children" in l) {
      ...
      for (; ++d < g.length; ) {
        const C = o.one(g[d], l);
        if (C) {
          if (d && g[d - 1].type === "break" && (!Array.isArray(C) && C.type === "text" && (C.value = Oa(C.value)), !Array.isArray(C) && C.type === "element")) {
            const k = C.children[0];
            k && k.type === "text" && (k.value = Oa(k.value));
          }
...
function Pa(t, e) {
  const n = Z1(t, e), r = n.one(t, void 0), i = z1(n), a = Array.isArray(r) ? { type: "root", children: r } : r || { type: "root", children: [] };
...
var Yn = Object.prototype.hasOwnProperty, to = Object.prototype.toString, Fa = Object.defineProperty, Ha = Object.getOwnPropertyDescriptor, za = function(e) {
  return typeof Array.isArray == "function" ? Array.isArray(e) : to.call(e) === "[object Array]";
}, Ua = function(e) {
  if (!e || to.call(e) !== "[object Object]")
    return !1;
...
const kt = { basename: ip, dirname: ap, extname: sp, join: op, sep: "/" };
function ip(t, e) {
...
class no {
  ...
  constructor(e) {
    let n;
    e ? ni(e) ? n = { path: e } : typeof e == "string" || fp(e) ? n = { value: e } : n = e : n = {}, this.cwd = "cwd" in n ? "" : up.cwd(), this.data = {}, this.history = [], this.messages = [], this.value, this.map, this.result, this.stored;
    let r = -1;
    for (; ++r < Ir.length; ) {
      const a = Ir[r];
   ...
   */
  set basename(e) {
    Nr(e, "basename"), Mr(e, "basename"), this.path = kt.join(this.dirname || "", e);
  }
  /**
   ...
   */
  set dirname(e) {
    ja(this.basename, "dirname"), this.path = kt.join(e || "", this.basename);
  }
  /**
   ...
   */
  set extname(e) {
    if (Mr(e, "extname"), ja(this.dirname, "extname"), e) {
   ...
   */
  set path(e) {
    ni(e) && (e = dp(e)), Nr(e, "path"), this.path !== e && this.history.push(e);
  }
  /**
   ...
   */
  set stem(e) {
    Nr(e, "stem"), Mr(e, "stem"), this.path = kt.join(this.dirname || "", e + (this.extname || ""));
  }
...
class Ti extends gp {
   ...
   */
  constructor() {
    super("copy"), this.Compiler = void 0, this.Parser = void 0, this.attachers = [], this.compiler = void 0, this.freezeIndex = -1, this.frozen = void 0, this.namespace = {}, this.parser = void 0, this.transformers = np();
  }
  /**
   * Copy a processor.
   *
   ...
   */
  data(e, n) {
    return typeof e == "string" ? arguments.length === 2 ? (Or("data", this.frozen), this.namespace[e] = n, this) : mp.call(this.namespace, e) && this.namespace[e] || void 0 : e ? (Or("data", this.frozen), this.namespace = e, this) : this.namespace;
  }
  /**
   * Freeze a processor.
   *
  ...
  process(e, n) {
    const r = this;
    return this.freeze(), Ar("process", this.parser || this.Parser), Lr("process", this.compiler || this.Compiler), n ? i(void 0, n) : new Promise(i);
    function i(a, s) {
      const o = Wn(e), c = (
      ...
      function u(l, h) {
        l || !h ? s(l) : a ? a(h) : (x(n, "`done` is defined if `resolve` is not"), n(void 0, h));
      }
    }
  }
  /**
  ...
  processSync(e) {
    let n = !1, r;
    return this.freeze(), Ar("processSync", this.parser || this.Parser), Lr("processSync", this.compiler || this.Compiler), this.process(e, i), Ga("processSync", "process", n), x(r, "we either bailed on an error or have a tree"), r;
    function i(a, s) {
      n = !0, Da(a), r = s;
    }
  }
  ...
  run(e, n, r) {
    ...
    function a(s, o) {
      ...
      function u(l, h, g) {
        ...
        l ? o(l) : s ? s(d) : (x(r, "`done` is defined if `resolve` is not"), r(void 0, d, g));
      }
    }
  }
  /**
  ...
  runSync(e, n) {
    let r = !1, i;
    return this.run(e, n, a), Ga("runSync", "run", r), x(i, "we either bailed on an error or have a tree"), i;
    function a(s, o) {
      Da(s), i = o, r = !0;
    }
  }
  ...
  stringify(e, n) {
    ...
    return Lr("stringify", i), Va(e), i(e, r);
  }
  /**
  ...
  use(e, ...n) {
    ...
    function c(u, l) {
      ...
      if (g === -1)
        ...
        let [d, ...C] = l;
        const k = r[g][1];
        ti(k) && ti(d) && (d = Rr(!0, k, d)), r[g] = [u, d, ...C];
      }
    }
  }
}
const yp = new Ti().freeze();
function Ar(t, e) {
...
const xp = "https://github.com/remarkjs/react-markdown/blob/main/changelog.md", Wa = [], qa = { allowDangerousHtml: !0 }, kp = /^(https?|ircs?|mailto|xmpp)$/i, Tp = [
...
function Ka(t) {
  const e = Ep(t), n = bp(t);
  return _p(e.runSync(e.parse(n), n), t);
}
function Ep(t) {
  const e = t.rehypePlugins || Wa, n = t.remarkPlugins || Wa, r = t.remarkRehypeOptions ? { ...t.remarkRehypeOptions, ...qa } : qa;
  return yp().use(s1).use(n).use(ep, r).use(e);
}
function bp(t) {
  const e = t.children || "", n = new no();
  return typeof e == "string" ? n.value = e : Vr(
    "Unexpected value `" + e + "` for `children` prop, expected `string`"
  ), n;
}
function _p(t, e) {
  const n = e.allowedElements, r = e.allowElement, i = e.components, a = e.disallowedElements, s = e.skipHtml, o = e.unwrapDisallowed, c = e.urlTransform || vp;
  for (const l of Tp)
    Object.hasOwn(e, l.from) && Vr(
      "Unexpected `" + l.from + "` prop, " + (l.to ? "use `" + l.to + "` instead" : "remove it") + " (see <" + xp + "#" + l.id + "> for more info)"
    );
  return n && a && Vr(
  ...
  function u(l, h, g) {
    if (l.type === "raw" && g && typeof h == "number")
      return s ? g.children.splice(h, 1) : g.children[h] = { type: "text", value: l.value }, h;
    if (l.type === "element") {
      let d;
      for (d in Er)
        if (Object.hasOwn(Er, d) && Object.hasOwn(l.properties, d)) {
          const C = l.properties[d], k = Er[d];
          (k === null || k.includes(l.tagName)) && (l.properties[d] = c(String(C || ""), d, l));
        }
    }
    if (l.type === "element") {
      let d = n ? !n.includes(l.tagName) : a ? a.includes(l.tagName) : !1;
      if (!d && r && typeof h == "number" && (d = !r(l, h, g)), d && g && typeof h == "number")
        return o && l.children ? g.children.splice(h, 1, ...l.children) : g.children.splice(h, 1), h;
    }
  }
}
function vp(t) {
...
function Ip(t, e, n) {
  ...
  function o(u, l) {
    let h = -1, g;
    for (; ++h < l.length; ) {
      const d = l[h], C = g ? g.children : void 0;
      if (i(
        d,
  ...
  function c(u, l) {
    const h = l[l.length - 1], g = a[s][0], d = a[s][1];
    let C = 0;
    const I = h.children.indexOf(u);
    let T = !1, b = [];
    g.lastIndex = 0;
    let v = g.exec(u.value);
    for (; v; ) {
      const O = v.index, A = {
        index: v.index,
        input: v.input,
      ...
      let E = d(...v, A);
      if (typeof E == "string" && (E = E.length > 0 ? { type: "text", value: E } : void 0), E === !1 ? g.lastIndex = O + 1 : (C !== O && b.push({
        type: "text",
        value: u.value.slice(C, O)
      }), Array.isArray(E) ? b.push(...E) : E && b.push(E), C = O + v[0].length, T = !0), !g.global)
        break;
      v = g.exec(u.value);
    }
    return T ? (C < u.value.length && b.push({ type: "text", value: u.value.slice(C) }), h.children.splice(I, 1, ...b)) : b = [u], I + b.length;
  }
}
function Mp(t) {
  const e = [];
  ...
  for (; ++r < n.length; ) {
    const i = n[r];
    e.push([Np(i[0]), Ap(i[1])]);
  }
  return e;
}
function Np(t) {
  return typeof t == "string" ? new RegExp(Rp(t), "g") : t;
}
function Ap(t) {
...
function Pp({ children: t, isStreaming: e }) {
  const [n, r] = we(!0), [i, a] = we(!1), [s, o] = we("");
  wt.useEffect(() => {
...
function Fp({
  ...
  const { t: a } = jt(), s = () => {
    if (!r || !i) return null;
    const h = i.find((g) => g.name === r);
    return (h == null ? void 0 : h.description) || null;
  };
  let o;
  if (r != null && r.startsWith("lat_")) {
    const h = (u = n == null ? void 0 : n.parameters) == null ? void 0 : u.query, g = (l = n == null ? void 0 : n.parameters) == null ? void 0 : l.url;
    o = h || g || a("chat.tools.executing");
  } else
    o = s();
...
function ao({ size: t = 16, variant: e = "dots" }) {
...
const oo = ri(null);
function zp({ children: t, value: e }) {
  return /* @__PURE__ */ p(oo.Provider, { value: e, children: t });
}
function Ln() {
  const t = ii(oo);
  if (!t)
...
const lo = ({ onImageClick: t, className: e, style: n, title: r, ...i }) => {
  const [a, s] = we(!1), o = de(null);
  ...
  ({ message: t }) => {
    const {
      ...
      currentAssistantMessageIdRef: c,
      onRetryMessage: u
    } = Ln(), { t: l } = jt(), [h, g] = we(!1), [d, C] = we(!1), [k, I] = we(null), T = ae(async () => {
      try {
        await navigator.clipboard.writeText(t.content), g(!0), setTimeout(() => g(!1), 2e3);
      } catch (N) {
      ...
      I(null);
    }, []), A = () => /* @__PURE__ */ P("div", { className: "chat-wrapper__streaming-placeholder", children: [
      /* @__PURE__ */ p(ao, { size: 16, variant: "dots" }),
      /* @__PURE__ */ p("span", { children: l("chat.reasoning.thinking") })
    ] }), E = () => u && /* @__PURE__ */ p(
      "button",
      {
    ...
    ] }), B = ze(() => Bp(v), [v]), G = () => /* @__PURE__ */ p("div", { className: "chat-wrapper__regular-message chat-wrapper__assistant-message-container", children: /* @__PURE__ */ P("div", { className: "chat-wrapper__assistant-content-wrapper", children: [
      /* @__PURE__ */ p("div", { className: "chat-wrapper__markdown-content", children: /* @__PURE__ */ p(
        Ka,
        {
          components: B,
          remarkPlugins: [Za],
          children: Up(t.content)
        },
        `${t.id}-${t.isStreaming ? "streaming" : "final"}`
      ) }),
      U()
    ] }) }), Z = () => /* @__PURE__ */ P("div", { className: "chat-wrapper__regular-message", children: [
      /* @__PURE__ */ p("div", { className: "chat-wrapper__markdown-content", children: /* @__PURE__ */ p(
        Ka,
        {
          remarkPlugins: [Za],
      ...
      t.media && t.media.length > 0 && /* @__PURE__ */ p("div", { className: "chat-wrapper__media", children: t.media.map((N, z) => /* @__PURE__ */ p(
        lo,
        {
          src: N,
          ...
          onMouseEnter: (q) => {
            q.currentTarget.style.transform = "scale(1.02)", q.currentTarget.style.boxShadow = "0 4px 12px rgba(0, 0, 0, 0.15)";
          },
          onMouseLeave: (q) => {
            q.currentTarget.style.transform = "scale(1)", q.currentTarget.style.boxShadow = "";
          },
          title: "Click to view full size"
        },
        z
      )) })
    ] }), se = () => t.role === "assistant" && t.isStreaming && t.content === "" && t.id === c.current ? A() : t.role === "system" ? /* @__PURE__ */ p(Hp, { message: t }) : t.role === "assistant" ? G() : Z(), D = () => /* @__PURE__ */ P(Pp, { isStreaming: t.isStreaming || !1, children: [
      /* @__PURE__ */ p(
        ro,
        {
      ...
      /* @__PURE__ */ p(io, { children: i(t.content) })
    ] }), M = () => {
      var N;
      return /* @__PURE__ */ p(Dp, { isStreaming: t.isStreaming || !1, children: /* @__PURE__ */ p(
        Fp,
        {
          title: a(t.content, t.isStreaming),
          status: s(t.content, t.isStreaming),
          toolData: t.toolData,
          toolName: (N = t.toolData) == null ? void 0 : N.toolName,
          clientTools: o
        }
      ) });
    };
    return /* @__PURE__ */ P(Ht, { children: [
      /* @__PURE__ */ p(
        "div",
        {
          ...
          children: t.role === "reasoning" ? D() : t.role === "tooling" ? M() : /* @__PURE__ */ P(Ht, { children: [
...
const $p = ({ isVisible: t }) => t ? /* @__PURE__ */ p("div", { className: "chat-wrapper__message chat-wrapper__message--assistant", children: /* @__PURE__ */ p("div", { className: "chat-wrapper__thinking-bubble", children: /* @__PURE__ */ p("div", { className: "chat-wrapper__thinking-content", children: /* @__PURE__ */ P("div", { className: "chat-wrapper__thinking-dots", children: [
  /* @__PURE__ */ p("span", {}),
  /* @__PURE__ */ p("span", {}),
  /* @__PURE__ */ p("span", {})
] }) }) }) }) : null, uo = Mn((t, e) => {
  ...
  return /* @__PURE__ */ P("div", { className: "chat-wrapper__messages", children: [
    ...
    /* @__PURE__ */ p($p, { isVisible: r && !i }),
    /* @__PURE__ */ p("div", { ref: e })
  ] });
});
uo.displayName = "MessagesList";
const St = (...t) => t.filter(Boolean).join(" "), jp = () => /* @__PURE__ */ P(
  "svg",
  {
    width: "54",
    height: "55",
...
), Gp = ({ className: t, ...e }) => /* @__PURE__ */ p("form", { className: St("chat-wrapper__prompt-input", t), ...e }), ho = Mn(
  ({
    onChange: t,
    className: e,
...
const Wp = ({
  className: t,
  ...e
}) => /* @__PURE__ */ p("div", { className: St("chat-wrapper__prompt-toolbar", t), ...e }), qp = ({
  className: t,
  ...e
}) => /* @__PURE__ */ p("div", { className: St("chat-wrapper__prompt-tools", t), ...e }), Kp = ({
  variant: t = "ghost",
  size: e = "default",
  className: n,
  children: r,
  ...
  const o = Pr(r);
  let c = o ? /* @__PURE__ */ p(Vp, {}) : /* @__PURE__ */ p(jp, {});
  return /* @__PURE__ */ p(
    "button",
    {
      className: St(
  ...
  children: e,
  ...n
}) => /* @__PURE__ */ p("select", { className: St("chat-wrapper__prompt-select", t), ...n, children: e }), Tf = ({
  className: t,
  children: e,
  ...n
}) => /* @__PURE__ */ p(
  ...
  const [r, i] = we(0), [a, s] = we(!1), [o, c] = we(0);
  return ke(() => {
  ...
  const {
    ...
    onFileUpload: g,
    onStopGeneration: d
  } = Ln(), { t: C } = jt(), k = r || i, I = l.length > 0, [T, b] = we(""), [v, O] = we([]), [A, E] = we([]), [U, B] = we(null), [G, Z] = we(null), [se, D] = we(!1), M = de(null), N = ae((W) => {
    Z(W), D(!0);
  }, []), z = ae((W) => new Promise((y, ue) => {
    const $ = new FileReader();
    $.onload = () => y($.result), $.onerror = ue, $.readAsDataURL(W);
  }), []), q = n && n.length > 0 ? n : [C("chat.input.placeholder")], te = T.length === 0 && !I && q.length > 1;
  Ja(
    e,
    () => ({
      focus: () => {
  ...
  const ge = ae(
    (W) => {
      W.preventDefault();
      const ue = new FormData(W.currentTarget).get("message");
      if (ue != null && ue.trim()) {
        const $ = si(ue.trim(), !1);
        if (!$.trim())
          return;
    ...
    (W) => {
      const ue = W.target.value.replace(/<script[\s\S]*?<\/script>/gi, "").replace(/javascript:/gi, "").replace(/on\w+\s*=/gi, "");
      b(ue), U && ue.trim() && B(null);
    },
    [U]
  ), w = ae(
    async (W) => {
      var $;
      const ue = Array.from((($ = W.clipboardData) == null ? void 0 : $.items) || []).filter((oe) => oe.type.startsWith("image/"));
      if (ue.length > 0) {
        W.preventDefault(), B(null);
        try {
          const oe = await Promise.all(
            ue.map((re) => {
              const Q = re.getAsFile();
              return Q ? new File(
                [Q],
          ...
          ).then((re) => re.filter(Boolean));
          if (oe.length > 0) {
            const re = oe.filter((Q) => {
              ...
              return Q.size > Te ? (B(
                ...
                "image/gif",
                "image/webp"
              ]).includes(Q.type) ? !0 : (B(C("chat.fileUpload.typeNotAllowed")), !1);
            });
            if (re.length > 0) {
              const Q = (o == null ? void 0 : o.maxFiles) ?? 5;
              if (v.length + A.length + re.length > Q) {
                B(
                  C("chat.fileUpload.maxFilesExceeded", { maxFiles: Q })
                );
                return;
              }
              const me = re.map(async (Ie) => ({
                file: Ie,
                preview: await z(Ie),
                isUploading: !0,
                progress: 0
              })), ye = await Promise.all(me);
              E((Ie) => [...Ie, ...ye]);
              try {
                const Ie = await g(re);
                E(
                  (Ue) => Ue.filter((at) => !re.includes(at.file))
                ), O((Ue) => [...Ue, ...Ie]), B(null);
              } catch {
                E(
                  (Ue) => Ue.filter((at) => !re.includes(at.file))
          ...
          B(
            oe instanceof Error ? oe.message : C("chat.errors.unexpected")
          ), E([]);
        }
      }
    },
    ...
    W.type = "file", W.accept = "image/*", W.multiple = !0, W.onchange = async (y) => {
      const ue = y.target.files;
      if (ue)
        try {
          B(null);
          const $ = Array.from(ue).filter((oe) => {
            const re = il(oe.name);
            oe.name;
            const Q = (o == null ? void 0 : o.maxFileSize) ?? 15 * 1024 * 1024;
            return oe.size > Q ? (B(
              C("chat.fileUpload.sizeLimitExceeded", {
                maxSize: Math.round(Q / (1024 * 1024))
              })
            ), !1) : ((o == null ? void 0 : o.allowedTypes) ?? [
              "image/jpeg",
              "image/png",
              "image/gif",
              "image/webp"
            ]).includes(oe.type) ? !0 : (B(C("chat.fileUpload.typeNotAllowed")), !1);
          });
          if ($.length > 0) {
            const oe = (o == null ? void 0 : o.maxFiles) ?? 5;
            if (v.length + A.length + $.length > oe) {
              B(
                C("chat.fileUpload.maxFilesExceeded", { maxFiles: oe })
              );
              return;
            }
            const Q = $.map(async (me) => ({
              file: me,
              preview: await z(me),
              isUploading: !0,
              progress: 0
            })), Te = await Promise.all(Q);
            E((me) => [...me, ...Te]);
            try {
              const me = await g($);
              E(
                (ye) => ye.filter((Ie) => !$.includes(Ie.file))
              ), O((ye) => [...ye, ...me]), B(null);
            } catch {
              E(
                (ye) => ye.filter((Ie) => !$.includes(Ie.file))
          ...
          B(
            $ instanceof Error ? $.message : C("chat.errors.unexpected")
          ), E([]);
        }
    }, W.click();
  }, [
  ...
  return /* @__PURE__ */ P(
    ...
    {
      onSubmit: ge,
      className: `${k ? "chat-wrapper__prompt-input--disabled" : ""} ${v.length > 0 || A.length > 0 || U ? "chat-wrapper__prompt-input--with-media" : ""}`,
      children: [
        /* @__PURE__ */ p(
          ho,
          {
        ...
        (v.length > 0 || A.length > 0) && /* @__PURE__ */ P("div", { className: "chat-wrapper__media-preview-container", children: [
          A.map((W, y) => /* @__PURE__ */ P(
            "div",
            {
          ...
          v.map((W, y) => {
            const ue = W.startsWith("data:image/"), $ = W.startsWith("http://") || W.startsWith("https://"), oe = ue || $;
            return /* @__PURE__ */ P(
              "div",
              {
                className: "chat-wrapper__media-item-wrapper",
                  ...
                  oe ? /* @__PURE__ */ P(
                    ...
                    /* @__PURE__ */ P("div", { className: "chat-wrapper__file-info", children: [
                      /* @__PURE__ */ p("div", { className: "chat-wrapper__file-name", children: (() => {
                        const re = W.match(/name=([^;]+)/);
                        return re ? decodeURIComponent(re[1]) : "document.pdf";
                      })() }),
                      /* @__PURE__ */ p("div", { className: "chat-wrapper__file-type", children: (() => {
                        const re = W.match(/data:([^;]+)/);
                        if (re) {
                          const Q = re[1];
                          switch (Q) {
        ...
        /* @__PURE__ */ P(Wp, { children: [
          /* @__PURE__ */ P(qp, { children: [
            s && /* @__PURE__ */ p("div", { className: "chat-wrapper__file-upload-container", children: /* @__PURE__ */ p(
              Kp,
              {
                ...
                size: "icon",
                onClick: Y,
                title: A.length > 0 ? `Uploading ${A.length} file(s)...` : v.length > 0 ? `${v.length}/${(o == null ? void 0 : o.maxFiles) ?? 5} image(s) attached` : `Attach images (max ${(o == null ? void 0 : o.maxFiles) ?? 5} files, ${Math.round(
                  ((o == null ? void 0 : o.maxFileSize) ?? 15 * 1024 * 1024) / (1024 * 1024)
                )}MB each)`,
                disabled: k || A.length > 0,
                children: /* @__PURE__ */ P(
                  "svg",
                  {
                    width: "36",
            ...
            s && c && /* @__PURE__ */ p("div", { className: "chat-wrapper__divider" }),
          ...
          /* @__PURE__ */ p(
            ...
            {
              status: a,
              disabled: Pr(a) ? !1 : !T.trim() || k || A.length > 0,
              onClick: Pr(a) && d ? () => {
                d();
              } : void 0
            }
  ...
  const { suggestedPrompts: t, chatInputRef: e, enableSuggestedPromptsAnimation: n = !0 } = Ln(), r = de(!1), i = de(null), a = de(null);
  if (ke(() => () => {
    ...
    return o.description.length > 0 && e.current.setText(o.description[0]), a.current = setTimeout(() => {
      ...
      const g = 10, d = () => {
        ...
        if (h < o.description.length) {
          ...
          c.dispatchEvent(k), h++, a.current = setTimeout(d, g);
        } else
  ...
  return /* @__PURE__ */ P("div", { className: "chat-wrapper__suggested-prompts", children: [
    ...
    /* @__PURE__ */ p("div", { className: "chat-wrapper__suggested-prompts-grid", children: t.map((o, c) => /* @__PURE__ */ p(
      "button",
      {
        className: "chat-wrapper__suggested-prompt-card",
        onClick: () => s(o),
        children: /* @__PURE__ */ P("div", { className: "chat-wrapper__suggested-prompt-content", children: [
          /* @__PURE__ */ p("h4", { className: "chat-wrapper__suggested-prompt-title", children: o.title }),
          /* @__PURE__ */ p("p", { className: "chat-wrapper__suggested-prompt-description", children: o.description })
        ] })
      },
      c
    )) })
  ...
  "div",
  {
    ...
    children: /* @__PURE__ */ p("div", { className: "chat-wrapper__inline-loader-content", children: /* @__PURE__ */ p(ao, { size: t, variant: "dots" }) })
  }
), ef = () => /* @__PURE__ */ P("svg", { width: "24", height: "24", viewBox: "0 0 24 24", fill: "none", xmlns: "http://www.w3.org/2000/svg", children: [
  ...
  n && /* @__PURE__ */ p("div", { className: "chat-wrapper__main-header-icon", children: /* @__PURE__ */ p(ef, {}) }),
  /* @__PURE__ */ p("h1", { className: "chat-wrapper__main-title", children: t }),
  e && /* @__PURE__ */ p("p", { className: "chat-wrapper__description", children: e })
] }), tf = () => /* @__PURE__ */ P("div", { className: "chat-wrapper__skeleton", children: [
  ...
  return /* @__PURE__ */ p("div", { className: "chat-wrapper__connection-error-overlay", children: /* @__PURE__ */ P("div", { className: "chat-wrapper__connection-error-card", children: [
    ...
    /* @__PURE__ */ p("p", { className: "chat-wrapper__connection-error-message", children: c }),
  ...
  const {
    ...
    conversationError: d,
    onRetryConnection: C
  } = Ln(), k = t.length === 0 && !e && h === He.CONNECTING, I = t.length === 0 && !e && h === He.DISCONNECTED && !g;
  if (k || I)
  ...
  const T = Et.state.shouldShowMainHeader(
    t.length,
    n,
    e
  ), b = Et.state.shouldShowSuggestedPrompts(
    t.length,
    n,
    e,
    a,
    s
  ), v = Et.state.getContentAreaClass(
    t.length,
    n,
    e,
    b
  ), O = v.includes("compact");
  return /* @__PURE__ */ P(Ht, { children: [
    ...
    /* @__PURE__ */ P(
      ...
      {
        className: v,
        style: l && t.length > 0 ? { paddingTop: "72px" } : void 0,
        children: [
          T && O && /* @__PURE__ */ p("div", { style: {
            flex: "1 1 auto",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            paddingTop: l ? "48px" : void 0
          }, children: /* @__PURE__ */ p(
            Ya,
            {
              headerName: r,
          ...
          !O && (e && t.length === 0 ? /* @__PURE__ */ p("div", { className: "chat-wrapper__messages", children: /* @__PURE__ */ p(Qp, { fullHeight: !0 }) }) : /* @__PURE__ */ p(uo, { ref: c })),
          /* @__PURE__ */ p("div", { className: "chat-wrapper__input-container", children: /* @__PURE__ */ p(Xp, { ref: u }) }),
          b && /* @__PURE__ */ p(Jp, {}),
...
const po = Mn(
  ({
    ...
    var Hn, ut;
    const { token: u, entityId: l, entityType: h } = t;
    Et.validation.validateAuthProps({
      userMpAuthToken: u,
      chatServerUrl: e,
      chatServerKey: n
    });
    const g = ze(() => Et.url.convertWebSocketToHttp(e), [e]), d = ze(
      () => {
        var L, xe;
        return new _c({
          apiUrl: g,
          ...
          allowedTypes: (xe = a.fileUploadConfig) == null ? void 0 : xe.allowedTypes
        });
      },
      [g, u, n, a.fileUploadConfig]
    ), C = ze(() => s && s.length > 0 ? s.map(({ execute: L, ...xe }) => xe) : [], [s]), k = kc(), { isOnline: I, wasOffline: T } = bc(), b = de(!0), v = le((L) => L.isModalOpen), O = le((L) => L.isCollapsed), A = le((L) => L.currentMode), E = le((L) => L.openModal), U = le((L) => L.closeModal), B = le((L) => L.toggleCollapse), G = le((L) => L.toggleFullscreen), Z = le((L) => L.setCurrentMode), se = le((L) => L.chatStatus), D = le((L) => L.setChatStatus), M = le((L) => L.streamingStatus), N = le((L) => L.setStreamingStatus), z = le(
      (L) => L.isLoadingConversation
    ), q = le(
      (L) => L.setIsLoadingConversation
    ), te = le((L) => L.conversationError), ge = le(
      (L) => L.setConversationError
    ), Se = le((L) => L.setCurrentThreadId), w = le((L) => L.providerResId), Y = le((L) => L.setProviderResId), W = le((L) => L.isStreaming), y = le((L) => L.setIsStreaming), ue = le((L) => L.isThinking), $ = le((L) => L.setIsThinking), oe = le((L) => L.streamingContent), re = le(
      (L) => L.setStreamingContent
    ), Q = le((L) => L.isHandlingTool), Te = le((L) => L.setIsHandlingTool);
    ke(() => {
      a.mode && Z(a.mode);
    }, [a.mode, Z]), ke(() => {
      if (typeof window > "u" || typeof document > "u")
    ...
    const {
      ...
      getReasoningStatus: Ue,
      getReasoningDuration: at,
      getReasoningContentOnly: st,
      getReasoningTitle: Rt,
      getToolingTitle: It,
      getToolingStatus: yt,
      handleSetMessage: Mt,
      handleReasoningUpdate: Nt,
      handleChatFinished: Gt,
      handleChatError: Wt,
      stopGeneration: Jt
    } = k, At = de(null), ot = de(null), _ = de(!1), R = de(null), V = ae(
      (L) => {
        Y(L.providerResId), Se(L.threadId);
      },
      [Y, Se]
    ), he = ae(
      (L) => {
        var xe, Ye;
        switch (L.type) {
          case ft.CHAT_COMPLETED:
            (xe = L.data) != null && xe.conversationId && Y(L.data.conversationId), Gt(), D(be.IDLE), N(dt.IDLE), setTimeout(() => {
              var Pe;
              (Pe = ot.current) == null || Pe.focus();
            }, 0);
            break;
          case ft.CHAT_ERROR:
            (Ye = L.data) != null && Ye.error && Wt(L.data.error);
            break;
          case ft.CONNECTION_LOST:
            break;
          case ft.CONNECTION_RESTORED:
            break;
          case ft.RECONNECTING:
            break;
        }
      },
      [
    ...
    ke(() => {
      ...
      metadata: r,
      chatClient: ne,
      currentProviderResId: w,
      isLoadingConversation: z,
      messages: me,
      entityId: l,
      entityType: h
    }), ke(() => {
      T && I && b.current ? Le().catch((L) => {
        const xe = ln(
          L,
          "NetworkReconnection"
        );
        b.current = xe.isRetryable, xe.isRetryable || console.warn(
          `[ChatWrapper] Network reconnection failed with non-retryable error: ${xe.reason}`
        );
      }) : T && I && !b.current && console.warn(
        "[ChatWrapper] Network restored but last error was non-retryable (CORS/auth), skipping reconnection"
      );
    }, [I, T, Le]);
    const ct = ae(() => {
      Jt(), D(be.IDLE), N(dt.IDLE), ne && w && ne.stopRun(w);
    }, [
      Jt,
      D,
      N,
    ...
    Ja(
      ...
      () => ({
        updateMetadata: (L) => {
          ne && w && ne.updateMetadata(w, L).catch((xe) => {
          });
        }
      }),
      [ne, w]
    ...
    ke(() => {
      ...
      a.onStreamingStatusChange && a.onStreamingStatusChange(M);
    }, [M, a]), ke(() => () => {
      Ve.current && cancelAnimationFrame(Ve.current);
    }, []), ke(() => () => {
      ye([]), y(!1), $(!1), re(""), Te(!1), D(be.IDLE), N(dt.IDLE), q(!1), ge(null), Se(null), Y(null);
    }, [
      ye,
      y,
      $,
    ...
    const dn = ae(
      async (L, xe) => {
        if (!L.trim() || W || !Be || !ne)
          return;
        y(!0), $(!0), D(be.SUBMITTED), N(dt.STARTING);
        const Ye = Be.createUserMessage(
          L,
          xe
        );
        if (ye((Ne) => [...Ne, Ye]), a.onConversationInitialized && !_.current && (_.current = !0, a.onConversationInitialized()), !navigator.onLine) {
          $(!1), D(be.ERROR), ye(
            (Ne) => Ne.map(
              (Re) => Re.id === Ye.id ? {
                ...Re,
          ...
          ), y(!1), D(be.IDLE), N(dt.IDLE);
          return;
        }
        try {
          const Ne = new Promise((Ot, Ut) => {
          ...
          $(!1), D(be.ERROR), ye(
            (Re) => Re.map(
              (Ot) => Ot.id === Ye.id ? {
                ...
                errorMessage: Ee !== He.CONNECTED ? "Connection lost. Message not sent." : Ne instanceof Error ? Ne.message : "Failed to send message. Please try again."
              } : Ot
            )
          ), y(!1), D(be.IDLE), N(dt.IDLE);
        }
      ...
      () => {
        ...
        return {
          ...
          fileUploadConfig: {
            maxFiles: ((xe = a.fileUploadConfig) == null ? void 0 : xe.maxFiles) ?? 5,
            maxFileSize: ((Ye = a.fileUploadConfig) == null ? void 0 : Ye.maxFileSize) ?? 15 * 1024 * 1024,
            // 15MB default
            allowedTypes: ((Pe = a.fileUploadConfig) == null ? void 0 : Pe.allowedTypes) ?? [
              "image/jpeg",
              "image/png",
              "image/gif",
              "image/webp"
      ...
      [
        ...
        (Hn = a.features) == null ? void 0 : Hn.fileUpload,
        a.fileUploadConfig,
        C
      ]
    ), Lt = ze(
      ...
      async (L) => {
        const xe = me.find((Pe) => Pe.id === L);
        if (!xe)
          return;
        if (y(!0), $(!0), D(be.SUBMITTED), N(dt.STARTING), ye((Pe) => Pe.map(
          (Ne) => Ne.id === L ? {
            ...Ne,
            hasError: !1,
            isRetrying: !0,
        ...
        try {
          Ee !== He.CONNECTED && await Le(), await (ne == null ? void 0 : ne.onTriggerMessage({
            message: xe.content,
            media: xe.media,
            providerResId: w || void 0,
            mcpHeaders: i
          ...
          $(!1), y(!1), D(be.ERROR), N(dt.IDLE), ye(
            (Ne) => Ne.map(
              (Re) => Re.id === L ? {
                ...Re,
                isRetrying: !1,
                hasError: !0,
                errorMessage: Pe instanceof Error ? Pe.message : "Retry failed. Please try again."
              } : Re
            )
          ), D(be.IDLE);
        }
    ...
    return ze(
      ...
      [A, v, O]
    ) ? /* @__PURE__ */ p(na, { children: /* @__PURE__ */ p(
      Fc,
      {
        mode: A,
        headerName: a.headerName,
        bubbleText: a.bubbleText,
        showBubbleText: ((ut = a.features) == null ? void 0 : ut.showBubbleText) !== !1,
        onClick: cr
      }
    ) }) : /* @__PURE__ */ p(na, { children: /* @__PURE__ */ p(
      Mc,
      {
        ...
        children: /* @__PURE__ */ P("div", { className: Qt, style: a.customStyles, children: [
          /* @__PURE__ */ p(
            af,
            {
              isVisible: !I,
              isReconnecting: Ee === He.RECONNECTING
            }
          ),
          Et.state.shouldShowHeader(a.headerVisible) && /* @__PURE__ */ p(
            Hc,
            {
              headerName: a.headerName,
              mode: A,
          ...
          !O && /* @__PURE__ */ p(
            Nc,
            {
              ...
              children: /* @__PURE__ */ p(zp, { value: Fn, children: /* @__PURE__ */ p(rf, {}) })
            }
          )
        ] })
      }
...
fo.displayName = "ChatWrapperContainer";
const vf = Xa(fo);
function Rf({
  isConnected: t,
  isConnecting: e = !1,
  ...
  return d ? o === "connecting" ? /* @__PURE__ */ p("div", { className: `connection-notification connection-notification--${o}`, children: /* @__PURE__ */ P("div", { className: "connection-notification__bubbles", children: [
    ...
    /* @__PURE__ */ p("div", { className: "connection-notification__icon", children: d.icon }),
    /* @__PURE__ */ p("div", { className: "connection-notification__title", children: d.title }),
    /* @__PURE__ */ p("div", { className: "connection-notification__message", children: d.message }),
...
export {
  ...
  Lc as CloseIcon,
  Pc as CollapseIcon,
  nf as ConnectionError,
  Rf as ConnectionNotification,
  Dc as CopyIcon,
  wo as EntityType,
  ...
  Gp as PromptInput,
  Kp as PromptInputButton,
  kf as PromptInputModelSelect,
  Ef as PromptInputModelSelectContent,
  bf as PromptInputModelSelectItem,
  Tf as PromptInputModelSelectTrigger,
  _f as PromptInputModelSelectValue,
  Zp as PromptInputSubmit,
  ho as PromptInputTextarea,
  Wp as PromptInputToolbar,
  qp as PromptInputTools,
  Pp as Reasoning,
  io as ReasoningContent,
  ro as ReasoningTrigger,
  dt as STREAMING_STATUS,
  xf as SettingsIcon,
  Jp as SuggestedPrompts,
  Cc as TranslationProvider,
  Mo as fetchThreadMessages,
  fs as fetchTranslations,
  Pr as isChatActive,
  hf as isChatError,
  uf as isChatIdle,
  df as isProcessingActive,
  pf as isProcessingComplete,
  ff as isProcessingError,
  No as updateThread,
  Ao as updateThreadMetadata,
  mf as useChatState,
  yf as useConversationState,
  wf as useI18next,
  gf as useLayoutState,
  Cf as useThreadState,
  jt as useTranslations,
  Sf as useUIState,
  le as useUIStore
};
